...
var myChart = new Chart(ctx, {
 ...
 showDBSalepersonShop: async function(range=""){
    ...
    for (let key in saleList) {
        ...
        for(let i=0;saleList[key].length > i;i++){
           ...
           // let orderCountWS = await this.loadOrderCount(val.shop_id,val.salesperson);

            var sum = (giftCardValue(val.shop_id).kortVal*1)*(val.antal*1);
            let orderCountWS =  val.orderantal*1;
            let db = (giftCardValue(val.shop_id).db*1)*(val.antal*1);
            let avgCardsPrSale =  (val.antal*1) / (orderCountWS*1);

            html+="<tr class='"+val.salesperson.toLowerCase()+"'><td>"+val.salesperson.toLowerCase()+"</td><td>"+giftCardValue(val.shop_id).kort+"</td><td  id='solgt"+val.shop_id+"'>"+val.antal+"</td><td>"+orderCountWS+"</td><td>"+ numberWithCommas(sum) +"</td> <td >"+numberWithCommas(db)+"</td><td >"+avgCardsPrSale.toFixed(0)+"</td></tr>"
            ...
            let tempdata = [
                ...
                giftCardValue(val.shop_id).kort,
                val.antal,
                orderCountWS,
 ...
 showTotal: async function(range=""){
     ...
     if(_country == "se"){
         ...
         console.log(response)
         /*
         jgvRes = await this.loadJgvTotal(range,1832);
         let jgvFiltedData = this.handelJgvData(jgvRes,1832,"Svensk");
         response = this.processArrayShowTotal(response,jgvFiltedData,1832);
         console.log(response)
     ...
     $(response.data.soldcard).each(function( index,val ) {
        ...
        let db = (giftCardValue(val.shop_id).db*1)*(val.antal*1);
        html+="<tr><td>"+giftCardValue(val.shop_id).kort+"</td><td  id='solgt"+val.shop_id+"'>"+val.antal+"</td><td>"+ numberWithCommas(sum)+"</td> <td >"+numberWithCommas(db)+"</td></tr>"
        totalval+=sum*1;
        totalDB+=db*1;
        totalCardSold+=val.antal*1;
        let tempdata = [
...
function giftCardValue(id)
{
    ...
    switch(id) {
                    ...
                    break;
                    case "2395":
                        return {kort:"Guldgavekortet-1040",kortVal:960,db:300,lang:1};
                    break;
                    case "2548":
                    ...
                    case "1832":
                        return {kort:"Svensk jgk 400/440",kortVal:400,db:100,lang:3};
                    break
                    case "1832_400":
                    ...
                    break
                    case "5117":
                        return {kort:"Svensk jgk 600",kortVal:600,db:100,lang:3};
