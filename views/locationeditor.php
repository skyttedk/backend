    ...
    function loadAttributeTexts()
    {
        ...
        else if(parseInt($('#locationType').val()) == 2)
        {
            ...
            $.post('index.php?rt=shop/locationattributes', {id: _editShopID, attribute_id: $('#locationAttributeSelect').val()}, function (response) {
                ...
                // Nulstil locationsort i locAvailableLocations
                $locationSort.html('');

                // Opret et sæt for hurtigt opslag af eksisterende lokationer
                var existingLocationsSet = new Set();

                // Fyld sættet med eksisterende lokationer fra alle adressers locationsort
                $('.locationsort .name').each(function() {
                    existingLocationsSet.add($(this).text());
                });

                // Tilføj lokationer til locationsort, hvis de ikke allerede findes
