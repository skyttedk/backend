...
.product-table th.sorting:after {
    content: '↕';
    position: absolute;
    right: 8px;
    color: #666;
...
.product-table th.sorting-asc:after {
    content: '↑';
}

.product-table th.sorting-desc:after {
    content: '↓';
}
...
function qrsettings(element){
    if($(element).attr("id") == "startRegistration"){
                 if( $('#startRegistration').is(":checked") ){
                       ajax({"shop_id":_editShopID,"token":_token},"shop/openForRegistration","msgOpenReg");
                 } else {
                      ajax({"shop_id":_editShopID,"token":_token},"shop/closeForRegistration","msgCloseReg");
                 }
    }
    if($(element).attr("id") == "allwaysShowRegistration"){
                 if( $('#allwaysShowRegistration').is(":checked") ){
                       ajax({"shop_id":_editShopID,"token":_token,"qrMenuSettings":"1"},"shop/setQrMenuSettings","QrMenuSettingsOpen");
                 } else {
                   ajax({"shop_id":_editShopID,"token":_token,"qrMenuSettings":"0"},"shop/setQrMenuSettings","QrMenuSettingsClose");
                 }
    }
}
...
function initSettings(response){
 ...
 //    console.log(response.data.attributes.kundepanel_email_regel )
   ...
   //   console.log(response.data.attributes.edit_allowed + "sdlakfjaæsldfj")
        if(_is_gf_user != 1){
           if(response.data.attributes.edit_allowed == "1"){
...
function qrLoadUser(){
        let postData = { "shopID":_editShopID,"token":_token };
        $.ajax(
            {
...
function buildQrTable(data){
 ...
 if($( ".gaveudlevering" ).hasClass( "levering_not_active" ) ){
        ajax({"shop_id":_editShopID,"token":_token},"shop/openForRegistration","openForRegistrationResponse");
         $(".qr_admin").show();
    } else {
          $(".qr_admin").hide();
   }
              ...
              <td valign="top">
                  <div class="product-container">
                      <table class="product-table">
                          <thead>
                          <tr>
                              <th data-sort="alias">Gave Alias</th>
                              <th data-sort="model_name">Gave</th>
                              <th data-sort="antal">Antal</th>
                              <th data-sort="procent">Procent</th>
                          </tr>
                          </thead>
                          <tbody>
                          </tbody>
