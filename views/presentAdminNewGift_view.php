...
<div id="presentsTabs">
	...
	<div id="tabs-1" style="height: 500px;">
      <div class="gaveAdminTab" style="width:700px;float:left;padding:2px">
          <div style="float: left"><label >PÅ LAGER</label> <input type="checkbox"  id="inStock"  /></div><br>
        <hr>
     ...
     <!--  <div style="width:150px;float:left"><label>Internt varenavn</label></div><div style="float:left;width:300px;height:30px;"><input value="" style="width:300px" type="text "  id="presentsAdminName" /></div><br /><br />-->
          <div style="width:100px;float:left; text-align: left;"><label>Godkendelse</label></div><div style="float:left !important;width:550px;">
              <div>
              <input type="checkbox"  class="godkendelse" id="Support24" value="Support24"/>
              <label >Support24</label> |
              <input type="checkbox" class="godkendelse" id="Tullin24" value="Tullin24"/>
              <label >Tullin24</label> |
              <input type="checkbox" class="godkendelse" id="MLH24" value="MLH24" />
              <label >MLH24</label>
                  <input type="checkbox" class="godkendelse" id="SEK24" value="SEK24" />
                  <label >SEK24</label> |
                  <input type="checkbox" class="godkendelse" id="ENG24" value="ENG24" />
                  <label >ENG24</label> |
                  <input type="checkbox" class="godkendelse" id="NOK24" value="NOK24" />
                  <label >NOK24</label>
              </div>
          </div> <br /><br />
        ...
        <div style="width:250px;float:left; text-align: left;"><label>Budget bruger kan se</label></div><div style="float:left;width:300px;height:30px;"><input  style="width:300px" type="text" min="0" id="presentsSubGiftList" /></div> <br />  <br />
        ...
        <div style="float:left;width:150px;height:30px;; text-align: left;"><label>Gave med omtanke: </label></div><div style="float:left;width:30px;height:30px;"><input type="checkbox" id="oko_present" ></div> <br />  <br />
         <div style="display:none; width:150px;float:left; text-align: left;"><label>Moms sats</label></div><div style="display:none;float:left;width:50px;height:30px;">
	...
	<div id="tabs-2">
        <div class="gaveAdminTab" id="presentDescriptionTabs" >
            ...
            <div style="display: inline-block">
                ...
                <div style="display: inline-block">
                    <label>Overskrift papirvalg:</label><br /><input type="text" size="50" id="presentsAdminHeadlineDaPaper" />
                </div>
...
<script src="views/js/logo.js?dd"></script>
<script>
var _dropTarget = "";
...
$(document).ready(function()
  {
    ...
    if(gaveEditData_ == "") {
      ...
      $('.godkendelse').click(function() {
        var checkedValues = $('.godkendelse:checked').map(function() {
                  return this.value;
              }).get().join(',');
          let postValue = {
              "option_value": checkedValues,
              "present_id": _selectedPresent
          };

          $.post("index.php?rt=presentoption/godkendelse", postValue, function(data, status){
              alert("Godkendelse ændret")
          });
      });

      $('#inStock').click(function() {

          let isCheck = $(this).is(':checked') ? 1:0
          let postValue = {
              "in_stock": isCheck,
              "present_id": _selectedPresent
          };

          $.post("index.php?rt=present/updateInStock", postValue, function(data, status){
              alert("Lagerstatus ændret")
          });

      });


      $.post("index.php?rt=presentoption/getGodkendelse", {"present_id": _selectedPresent}, function(data, status){
          let d = JSON.parse(data)
          if (d.data.result.length == 0) return;

          optionValue = d.data.result[0].option_value;

          var valuesToCheck = optionValue.split(',');

          // Itererer over alle checkboxes med klassen 'godkendelse'
          $('.godkendelse').each(function() {
              // Hvis værdien af den aktuelle checkbox findes i valuesToCheck arrayet, markerer den som checked
              if(valuesToCheck.indexOf($(this).val()) !== -1) {
