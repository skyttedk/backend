...
class CardShopPDF
{
    ...
    private function makePage($username,$password,$shopId,$deadline,$is_delivery){
        ...
        $shopSV = array("1832","1981","2558","4793","5117","8271");
        if(in_array($shopId, $shopSV)) {
            if($is_delivery == 1){
                $this->pdf->Image(GFConfig::BACKEND_PATH.'files/img/'.$this->getCardImgPath($shopId."_home"),5,50,200);
            } else {
                $this->pdf->Image(GFConfig::BACKEND_PATH.'files/img/'.$this->getCardImgPath($shopId),5,50,0);
            }
            ...
            if($shopId == "2960" || $shopId == "2961" || $shopId == "2962" || $shopId == "2963" || $shopId == "2964"){
                ...
                if ($is_delivery == 1 and in_array($shopId, $shopDA)) {
                    ...
                    $this->pdf->Image(GFConfig::BACKEND_PATH . 'files/img/' . $this->getCardImgPath($shopId), 5, 0, 200);
                }
                if(in_array($shopId, $shopDA)) {
                    $this->pdf->SetFont('Arial','',12);
                    $this->pdf->Text(78,150,'https://www.gavevalg.dk');
                    $this->pdf->SetFont('Arial','',12);
                    $this->pdf->Text(51,192,$username);
                    $this->pdf->Text(118,192,$password);
