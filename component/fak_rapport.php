...
GROUP BY company_order.order_no,`order`.present_model_id
order by company_order.order_no,shop_user.shop_id
...
$csv = "BS-nummer;Gave navn;Model navn;Varenr;Antal valgte"."\n";
//print_r($rs["data"] );
foreach($rs["data"]  as $key=>$val)
{
    $diff = getOrderQuantity($val["order_no"],$db) -  $sumTotal[$val["order_no"]];
