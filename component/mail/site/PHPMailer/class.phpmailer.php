...
class PHPMailer
{
    ...
    protected function addAnAddress($kind, $address, $name = '')
    {
        ...
        $address = trimgf($address);
        $name = trimgf(preg_replace('/[\r\n]+/', '', $name)); //Strip breaks and trim
    ...
    public function preSend()
    {
        try {
            $this->mailHeader = '';
            if ((count($this->to) + countgf($this->cc) + countgf($this->bcc)) < 1) {
            ...
            if ($this->Mailer == 'mail') {
                ...
                $this->mailHeader .= $this->headerLine(
                    'Subject',
                    $this->encodeHeader($this->secureHeader(trimgf($this->Subject)))
                );
            }
    ...
    protected function mailSend($header, $body)
    {
        ...
        $result = false;
        if ($this->SingleTo && countgf($toArr) > 1) {
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if ((count($this->all_recipients) > countgf($bad_rcpt)) and !$this->smtp->data($header . $body)) {
    ...
    public function smtpConnect($options = array())
    {
        ...
        foreach ($hosts as $hostentry) {
            $hostinfo = array();
            if (!preg_match('/^((ssl|tls):\/\/)*([a-zA-Z0-9\.-]*):?([0-9]*)$/', trimgf($hostentry), $hostinfo)) {
    ...
    public function wrapText($message, $length, $qp_mode = false)
    {
        ...
        $message = '';
        for ($i = 0; $i < countgf($line); $i++) {
            $line_part = explode(' ', $line[$i]);
            $buf = '';
            for ($e = 0; $e < countgf($line_part); $e++) {
                $word = $line_part[$e];
    ...
    public function createHeader()
    {
        ...
        $result .= $this->addrAppend('From', array(array(trimgf($this->From), $this->FromName)));
        ...
        if ($this->ConfirmReadingTo != '') {
            $result .= $this->headerLine('Disposition-Notification-To', '<' . trimgf($this->ConfirmReadingTo) . '>');
        }

        // Add custom headers
        for ($index = 0; $index < countgf($this->CustomHeader); $index++) {
            $result .= $this->headerLine(
                trimgf($this->CustomHeader[$index][0]),
                $this->encodeHeader(trimgf($this->CustomHeader[$index][1]))
            );
        }
    ...
    public function encodeHeader($str, $position = 'text')
    {
        ...
        if ($matchcount > strlen($str) / 3) {
            ...
            if (function_exists('mb_strlen') && $this->hasMultiBytes($str)) {
                ...
                $maxlen -= $maxlen % 4;
                $encoded = trimgf(chunk_split($encoded, $maxlen, "\n"));
            }
        } else {
            $encoding = 'Q';
    ...
    public function html2text($html, $advanced = false)
    {
        ...
        return html_entity_decode(
            trimgf(strip_tags(preg_replace('/<(head|title|style|script)[^>]*>.*?<\/\\1>/si', '', $html))),
            ENT_QUOTES,
            $this->CharSet
        );
    }
    ...
    {
        return trimgf(str_replace(array("\r", "\n"), '', $str));
    }
    ...
    public function DKIM_HeaderC($signHeader)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            $lines[$key] = $heading . ':' . trimgf($value); // Don't forget to remove WSP around the value
        }
        $signHeader = implode("\r\n", $lines);
        return $signHeader;
    }
