	...
	function extractInfo($debug = false, $BMPonly = false, $useOTL = 0)
	{
		...
		if ($kern_offset && $version == 0 && $format == 0) {
			...
			for ($i = 0; $i < $nPairs; $i++) {
				...
				$val = $this->read_short();
				if (count($glyphToChar[$left]) == 1 && countgf($glyphToChar[$right]) == 1) {
					if ($left != 32 && $right != 32) {
		...
		if (isset($this->tables["GDEF"])) {
			...
			if ($MarkAttachClassDef_offset) {
				...
				foreach ($MarkAttachmentTypes AS $class => $glyphs) {
					...
					if (is_array($Marks) && countgf($Marks)) {
				...
				for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
					...
					// LookupType 2: Multiple Substitution Subtable 1 => n
						...
						for ($g = 0; $g < countgf($glyphs); $g++) {
							...
							if (!isset($Lookup[$i]['Subtable'][$c]['Sequences'][$g]['SubstituteGlyphID']) || countgf($Lookup[$i]['Subtable'][$c]['Sequences'][$g]['SubstituteGlyphID']) == 0) {
								continue;
						...
						foreach ($volt AS $v) {
							...
							if (strpos('isol fina init medi fin2 fin3 med2', $v['tag']) !== false) {
								...
								if ($v['tag'] == 'isol')
									...
									$kk = 6;
								$rtl[$key][$kk] = $sub;
								if (isset($v['prel']) && countgf($v['prel']))
									$rtl[$key]['prel'][$kk] = $v['prel'];
								if (isset($v['postl']) && countgf($v['postl']))
					...
					if (strpos('dev2 bng2 gur2 gjr2 ory2 tml2 tel2 knd2 mlm2 deva beng guru gujr orya taml telu knda mlym', $st) !== false) { // all INDIC scripts [any/all languages]
						...
						foreach ($volt AS $v) {
							if (strpos('locl', $v['tag']) !== false) {
								...
								$key = preg_replace('/[\(\)]*/', '', $key);
								$sub = $v['replace'];
								if ($key && strlen(trimgf($key)) == 5 && $sub) {
									$loclsubs[$sub] = $key;
								}
							}
						}
						...
						foreach ($volt AS $v) {
							...
							if (strpos('rphf half pref blwf pstf', $v['tag']) !== false) {
								if (isset($v['context']) && $v['context'] && $v['nBacktrack'] == 0 && $v['nLookahead'] == 0) {
									foreach ($v['rules'] AS $vs) {
										if (count($vs['match']) == 2 && countgf($vs['replace']) == 1) {
											$sub = $vs['replace'][0];
									...
									$key = preg_replace('/[\(\)]*/', '', $key);
									$sub = $v['replace'];
									if ($key && strlen(trimgf($key)) == 11 && $sub) {
										// If Cons Halant    <rphf> and <half> always
										// and <pref>, <blwf> and <pstf> in Old version
										// If Halant Cons   <pref>, <blwf> and <pstf> in New version only
...
//print_r($rtlpua); exit;
					...
					//=====================================================================================
					//=====================================================================================
					if (count($rtl) || countgf($rphf) || countgf($half) || countgf($pref) || countgf($blwf) || countgf($pstf) || $finals) {
	...
	function _getGSUBarray(&$Lookup, &$lul, $scripttag)
	{
		...
		foreach ($lul AS $i => $tag) {
			for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
				...
				// LookupType 1: Single Substitution Subtable
				if ($Lookup[$i]['Type'] == 1) {
					for ($s = 0; $s < countgf($Lookup[$i]['Subtable'][$c]['subs']); $s++) {
				...
				// LookupType 4: Ligature Substitution Subtable
					...
					for ($s = 0; $s < countgf($Lookup[$i]['Subtable'][$c]['subs']); $s++) {
						...
						$subs = $this->_makeGSUBinputReplacement(count($inputGlyphs), $substitute, $ignore, 0, countgf($inputGlyphs), 0);
				...
				// LookupType 5: Chaining Contextual Substitution Subtable
						...
						foreach ($Lookup[$i]['Subtable'][$c]['SubClassSet'] AS $inputClass => $cscs) {
							for ($cscrule = 0; $cscrule < $cscs['SubClassRuleCnt']; $cscrule++) {
								...
								for ($b = 0; $b < $rule['SubstCount']; $b++) {
									...
									// $Lookup[$lup] = secondary Lookup
									for ($lus = 0; $lus < $Lookup[$lup]['SubtableCount']; $lus++) {
										if (isset($Lookup[$lup]['Subtable'][$lus]['subs']) && countgf($Lookup[$lup]['Subtable'][$lus]['subs'])) {
											foreach ($Lookup[$lup]['Subtable'][$lus]['subs'] AS $luss) {
					...
					// Format 3: Coverage-based Context Glyph Substitution  p259
						...
						$nBsubs = 2 * countgf($backtrackGlyphs);
						$nIsubs = (2 * $nInput) - 1;
						$contextInputMatch = $this->_makeGSUBcontextInputMatch($inputGlyphs, $ignore, array(), 0);
						$subRule = array('context' => 1, 'tag' => $tag, 'matchback' => $backtrackMatch, 'match' => ($contextInputMatch . $lookaheadMatch), 'nBacktrack' => countgf($backtrackGlyphs), 'nInput' => $nInput, 'nLookahead' => countgf($lookaheadGlyphs), 'rules' => array(),);
...
//print_r($Lookup); exit;
	...
	function _makeGSUBcontextInputMatch($inputGlyphs, $ignore, $lookupGlyphs, $seqIndex)
	{
		...
		$mLen = countgf($lookupGlyphs); // nGlyphs in the secondary Lookup match
		$nInput = countgf($inputGlyphs); // nGlyphs in the Primary Input sequence
		$str = "";
		for ($i = 0; $i < $nInput; $i++) {
			if ($i > 0) {
				$str .= $ignore . " ";
	...
	function _makeGSUBinputMatch($inputGlyphs, $ignore)
	{
		...
		$str = "";
		for ($i = 1; $i <= countgf($inputGlyphs); $i++) {
			if ($i > 1) {
				$str .= $ignore . " ";
			}
			$str .= "(" . $inputGlyphs[($i - 1)] . ")";
	...
	function _makeGSUBlookaheadMatch($lookaheadGlyphs, $ignore)
	{
		...
		// each item being e.g. E0AD|E0AF|F1FD
		$str = "";
		for ($i = 0; $i < countgf($lookaheadGlyphs); $i++) {
...
?>';
	...
	{ // mPDF 5.7.1
		...
		$numGlyphs = $numberOfHMetrics = countgf($subsetglyphs);
		...
		$segCount = countgf($range) + 1; // + 1 Last segment has missing character 0xFFFF
		$searchRange = 1;
		$entrySelector = 0;
	...
	{ // mPDF 5.7.1
		...
		foreach ($subset AS $code) {
			...
			if (!isset($glyphSet[$originalGlyphIdx])) {
				$glyphSet[$originalGlyphIdx] = countgf($glyphMap);
			...
			if ($numberOfContours < 0) {
				...
				while ($flags & GF_MORE) {
					...
					if (!isset($glyphSet[$glyphIdx])) {
						$glyphSet[$glyphIdx] = countgf($glyphMap);
						$glyphMap[] = $glyphIdx;
					}
					if ($flags & GF_WORDS)
						$this->skip(4);
		...
		if (isset($this->tables['OS/2'])) {
			...
			$os2 = $this->_set_ushort($os2, 66, countgf($subset)); // LastCharIndex
	...
	function getGlyphs($originalGlyphIdx, &$start, &$glyphSet, &$subsetglyphs)
	{
		...
		if ($numberOfContours < 0) {
			...
			while ($flags & GF_MORE) {
				...
				$glyphIdx = $this->read_ushort();
				if (!isset($glyphSet[$glyphIdx])) {
					$glyphSet[$glyphIdx] = countgf($subsetglyphs); // old glyphID to new glyphID
	...
	{
		$stm = '';
		$numTables = countgf($this->otables);
		$searchRange = 1;
		$entrySelector = 0;
	...
	{ // mPDF 5.7.1
		...
		if ($useOTL) {
			...
			foreach ($range AS $start => $subrange) {
				$idDelta = -($start - $subrange[0]);
				//$n += countgf($subrange);	// ?? Line not required
				$cmap[] = $idDelta; // idDelta(s)
			}
			$cmap[] = 1; // idDelta of last Segment
			// idRangeOffset(s)
