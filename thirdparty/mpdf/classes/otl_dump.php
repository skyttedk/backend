				...
				// LookupType 3: Alternate Forms
					...
					for ($s = 0; $s < countgf($Lookup[$i]['Subtable'][$c]['subs']); $s++) {
						...
						if (count($Lookup[$i]['Subtable'][$c]['subs'][$s]['substitute']) > 1) {
							for ($alt = 1; $alt < countgf($Lookup[$i]['Subtable'][$c]['subs'][$s]['substitute']); $alt++) {
					...
					if ($SubstFormat == 1) {
						...
						for ($s = 0; $s < $Lookup[$i]['Subtable'][$c]['SubRuleSetCount']; $s++) {
							...
							foreach ($Lookup[$i]['Subtable'][$c]['SubRuleSet'][$s]['SubRule'] AS $rctr => $rule) {
								...
								for ($ff = 0; $ff < countgf($inputGlyphs); $ff++) {
								...
								for ($b = 0; $b < $rule['SubstCount']; $b++) {
									...
									if (count($inputGlyphs) > ($seqIndex + 1)) {
										...
										for ($ip = $seqIndex + 1; $ip < countgf($inputGlyphs); $ip++) {
					...
					if ($SubstFormat == 1) {
						...
						for ($s = 0; $s < $Lookup[$i]['Subtable'][$c]['ChainSubRuleSetCount']; $s++) {
							...
							foreach ($Lookup[$i]['Subtable'][$c]['ChainSubRuleSet'][$s]['ChainSubRule'] AS $rctr => $rule) {
								...
								$exampleL = array();
								$html .= '<div class="context">CONTEXT: ';
								for ($ff = countgf($backtrackGlyphs) - 1; $ff >= 0; $ff--) {
								...
								for ($ff = 0; $ff < countgf($lookaheadGlyphs); $ff++) {
...
// Takes too long e.g. Calibri font - just list kerning pairs with this:
					...
					//===========
						...
						for ($i = 0; $i < $Class1Count; $i++) {
							for ($j = 0; $j < $Class2Count; $j++) {
								...
								for ($c1 = 0; $c1 < countgf($Class1[$i]); $c1++) {
									...
									for ($c2 = 0; $c2 < countgf($Class2[$j]); $c2++) {
				...
				////////////////////////////////////////////////////////////////////////////////
					...
					for ($i = 0; $i < countgf($MarkGlyphs); $i++) {
					...
					for ($j = 0; $j < countgf($BaseGlyphs); $j++) {
				...
				////////////////////////////////////////////////////////////////////////////////
					...
					for ($j = 0; $j < countgf($LigatureGlyphs); $j++) {
				...
				////////////////////////////////////////////////////////////////////////////////
					...
					$firstMark = '';
					$html .= '<div class="glyphs">Marks: <span class="unchanged">';
					for ($i = 0; $i < countgf($Mark1Glyphs); $i++) {
					...
					if ($firstMark) {
						...
						for ($j = 0; $j < countgf($Mark2Glyphs); $j++) {
