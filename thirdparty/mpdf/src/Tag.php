	...
	public function OpenTag($tag, $attr, &$ahtml, &$ihtml)
	{
		...
		switch ($tag) {
			...
			case 'BR':
				...
				if ($this->mpdf->tableLevel) {
					...
					if (count($this->mpdf->textbuffer)) {
						$this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0] = preg_replace(
							'/ $/',
							'',
							$this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0]
						);
						if (!empty($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][18])) {
							$this->otl->trimOTLdata($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][18], false, true);
						} // *OTL*
					}
			...
			case 'MAIN':
				...
				if ($this->mpdf->blockjustfinished && !countgf($this->mpdf->textbuffer)
			...
			case 'TABLE': // TABLE-BEGIN
				...
				} elseif (!$this->mpdf->tableLevel && countgf($this->mpdf->textbuffer)) {
					$this->mpdf->printbuffer($this->mpdf->textbuffer, $blockstate);
				}
	...
	// mPDF 6
			...
			if (count($this->mpdf->textbuffer) && $this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1]) {
				if (substr($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0], 0, 3) != "\xbb\xa4\xac") { // not special content
					...
					if (preg_match('/[ ]+$/', $this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0], $m)) {
						$strip = strlen($m[0]);
						$this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0] = substr(
							$this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0],
							0,
							(strlen($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0]) - $strip)
						);
						/* -- OTL -- */
						if (isset($this->mpdf->CurrentFont['useOTL']) && $this->mpdf->CurrentFont['useOTL']) {
							$this->otl->trimOTLdata($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][18], false, true); // mPDF 6  ZZZ99K
						}
						/* -- END OTL -- */
					}
				}
		...
		if ($tag == 'TABLE') { // TABLE-END (
			...
			if (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot'])
					&& countgf($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot'])) {
				$tfrows = [];
