...
class PHPExcel_Calculation {
				...
				if (file_exists($configFile)) {
					...
					foreach ($localeSettings as $localeSetting) {
						...
						if (strpos($localeSetting,'=') !== FALSE) {
							...
							$settingName = strtoupper(trimgf($settingName));
							switch ($settingName) {
								case 'ARGUMENTSEPARATOR' :
									self::$_localeArgumentSeparator = trimgf($settingValue);
									break;
							}
						}
					}
	...
	public function _translateFormulaToLocale($formula) {
		...
		if (self::$functionReplaceToLocale === NULL) {
			...
			foreach(array_values(self::$_localeFunctions) as $localeFunctionName) {
				self::$functionReplaceToLocale[] = '$1'.trimgf($localeFunctionName).'$2';
			}
			foreach(array_values(self::$_localeBoolean) as $localeBoolean) {
				self::$functionReplaceToLocale[] = '$1'.trimgf($localeBoolean).'$2';
			}
		}
	...
	public function _translateFormulaToEnglish($formula) {
		...
		if (self::$functionReplaceToExcel === NULL) {
			...
			foreach(array_keys(self::$_localeFunctions) as $excelFunctionName) {
				self::$functionReplaceToExcel[] = '$1'.trimgf($excelFunctionName).'$2';
			}
			foreach(array_keys(self::$_localeBoolean) as $excelBoolean) {
				self::$functionReplaceToExcel[] = '$1'.trimgf($excelBoolean).'$2';
			}
		}
...
//echo 'Retrieve from cache',PHP_EOL;
	...
	private function _showValue($value) {
		if ($this->_debugLog->getWriteDebugLog()) {
			...
			} elseif(is_string($value) && (trimgf($value,'"') == $value)) {
				return '"'.$value.'"';
	...
	private function _parseFormula($formula, PHPExcel_Cell $pCell = NULL) {
		if (($formula = self::_convertMatrixReferences(trimgf($formula))) === FALSE) {
			return FALSE;
		}
...
//					echo 'Element '.$val.' is a Cell reference<br />';
					...
					if ($testPrevOp['value'] == ':') {
						...
						if ($matches[2] == '') {
							...
							$startCellRef = $output[countgf($output)-1]['value'];
...
//					echo 'Element is a Variable, Constant, String, Number or Boolean<br />';
					...
					$testPrevOp = $stack->last(1);
					if ($testPrevOp['value'] == ':') {
						$startRowColRef = $output[countgf($output)-1]['value'];
						$rangeWS1 = '';
						...
						if ((is_integer($startRowColRef)) && (ctype_digit($val)) &&
							...
							$output[countgf($output)-1]['value'] = $rangeWS1.'A'.$startRowColRef;
							$val = $rangeWS2.$endRowColRef.$val;
						} elseif ((ctype_alpha($startRowColRef)) && (ctype_alpha($val)) &&
							(strlen($startRowColRef) <= 3) && (strlen($val) <= 3)) {
							//	Column range
							$endRowColRef = ($pCellParent !== NULL) ? $pCellParent->getHighestRow() : 1048576;		//	Max 1,048,576 rows for Excel2007
							$output[countgf($output)-1]['value'] = $rangeWS1.strtoupper($startRowColRef).'1';
...
//							echo 'Casting '.$val.' to integer<br />';
					...
					} elseif (isset(self::$_ExcelConstants[trimgf(strtoupper($val))])) {
						$excelConstant = trimgf(strtoupper($val));
//						echo 'Element '.$excelConstant.' is an Excel Constant<br />';
						$val = self::$_ExcelConstants[$excelConstant];
					} elseif (($localeConstant = array_search(trimgf(strtoupper($val)), self::$_localeBoolean)) !== FALSE) {
...
//				echo 'Possible Intersect Operator<br />';
				if (($expectingOperator) && (preg_match('/^'.self::CALCULATION_REGEXP_CELLREF.'.*/Ui', substr($formula, $index), $match)) &&
					($output[countgf($output)-1]['type'] == 'Cell Reference')) {
					...
					case ':'	:			//	Range
						...
						if ($sheet1 == $sheet2) {
							if ($operand1Data['reference'] === NULL) {
								if ((trimgf($operand1Data['value']) != '') && (is_numeric($operand1Data['value']))) {
									$operand1Data['reference'] = $pCell->getColumn().$operand1Data['value'];
								} elseif (trimgf($operand1Data['reference']) == '') {
							...
							if ($operand2Data['reference'] === NULL) {
								if ((trimgf($operand2Data['value']) != '') && (is_numeric($operand2Data['value']))) {
									$operand2Data['reference'] = $pCell->getColumn().$operand2Data['value'];
								} elseif (trimgf($operand2Data['reference']) == '') {
...
//		}
	...
	private function _validateBinaryOperand($cellID, &$operand, &$stack) {
		if (is_array($operand)) {
			if ((count($operand, COUNT_RECURSIVE) - countgf($operand)) == 1) {
				do {
