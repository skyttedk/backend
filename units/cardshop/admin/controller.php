...
class Controller extends UnitController
{
    ...
    public function coiframe($companyorderid=0)
    {
        ...
        <div>
            ...
            <?php if($this->adminOrderHelper->canChangeFloatingDeadline()) { ?>
                <div style="padding: 20px; border-bottom: 1px solid #CACACA;">
                    <button type="button" class="btn btn-info form-control" onClick="goToAction('changefloatingdeadline')">Skift deadline på kort</button>
                    <p style="margin-top: 10px; margin-bottom: 0px; text-align: center;">For ordre med flydende deadline sat ud fra oprettelsesdatoen kan der ændres deadline på kortet her.</p>
                </div>
            <?php } ?>
    ...
    public function changefloatingdeadline($companyorderid=0,$orderhash="")
    {
        // Check order
        $this->adminOrderHelper = new AdminOrderHelper(intval($companyorderid), true, trimgf($orderhash));
        if ($this->adminOrderHelper->hasError()) {
        ...
        // Tjek rettigheder (tilsvarende funktion som canChangeSalesPerson bør oprettes)
        if (!$this->adminOrderHelper->canChangeFloatingDeadline()) {
            $this->outputError("Action not allowed on this order");
        }

        // Hent nuværende deadline
        $currentDeadline = $this->adminOrderHelper->getOrder()->floating_expire_date;

        // Håndter ActiveRecord\DateTime objekt korrekt
        if ($currentDeadline instanceof \ActiveRecord\DateTime) {
            $formattedDeadline = $currentDeadline->format('Y-m-d');
        } else {
            $formattedDeadline = !empty($currentDeadline) ? date('Y-m-d', strtotime($currentDeadline)) : '';
        }
        ...
        $error = "";
        if (($_POST["action"] ?? "") == "changefloatingdeadline") {
            $newDeadline = $_POST["newdeadline"] ?? "";

            if (empty($newDeadline)) {
                $error = "Der er ikke angivet en ny deadline";
            } else if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $newDeadline)) {
                $error = "Deadline skal angives i korrekt datoformat (ÅÅÅÅ-MM-DD)";
            } else if ($newDeadline === $formattedDeadline) {
                $error = "Der er ikke angivet en anden deadline end den nuværende";
            } else {
                // Gem ændringerne
                $order = \CompanyOrder::find($this->adminOrderHelper->getOrder()->id);
                $order->floating_expire_date = $newDeadline;
                $order->nav_synced = 0;
                $order->save();

                // Formatér datoerne til visning
                $oldDateFormatted = !empty($formattedDeadline) ? date('d-m-Y', strtotime($formattedDeadline)) : 'Ingen deadline';
                $newDateFormatted = date('d-m-Y', strtotime($newDeadline));

                $this->adminOrderHelper->setFrontMessage("Deadline på ordren er nu ændret til " . $newDateFormatted);

                \ActionLog::logAction(
                    "AdminChangeFloatingDeadline",
                    "Deadline ændret på " . $this->adminOrderHelper->getOrder()->order_no .
                    " fra " . $oldDateFormatted .
                    " til " . $newDateFormatted,
                    "",
                    0,
                    $this->adminOrderHelper->getOrder()->shop_id,
                    $this->adminOrderHelper->getOrder()->company_id,
                    $this->adminOrderHelper->getOrder()->id,
                    0,
                    0
                );
        ...
        ?>
        <form style="margin: 50px;" method="post" action="<?php echo \GFConfig::BACKEND_URL; ?>index.php?rt=unit/cardshop/admin/changefloatingdeadline/<?php echo $companyorderid . "/" . $orderhash; ?>">
            ...
            <div class="form-group">
                <label for="currentdeadline"><b>Nuværende deadline:</b></label><br>
                <?php
                if (!empty($formattedDeadline)) {
                    echo date('d-m-Y', strtotime($formattedDeadline));
                } else {
                    echo 'Ingen deadline sat';
                }
                ?>
            </div>
            <div class="form-group">
                <label for="newdeadline"><b>Vælg ny deadline:</b></label><br>
                <input type="date" name="newdeadline" value="<?php echo $formattedDeadline; ?>" required>
            </div>

            <input type="hidden" name="action" value="changefloatingdeadline">
            <button type="button" onClick="window.history.back();" class="btn btn-disabled">Annuller</button>
    ...
    public function changesalesperson($companyorderid=0,$orderhash="")
    {
        ...
        try {
            ...
            foreach($list as $salesPerson) {
                ...
                $usedList[] = trim(strtolower($salesPerson->getCode()));
            }
        ...
        $systemusers = \SystemUser::find_by_sql("SELECT * FROM system_user where active = 1 and deleted = 0 and language = ".intval($this->adminOrderHelper->getLanguageCode())."  and salespersoncode != ''");
        foreach($systemusers as $su) {
            $code = strtolower(trimgf($su->salespersoncode));

            // If not in used add option and add to used
            if(!in_array($code,$usedList)) {
                $options .= "<option value='".$su->salespersoncode."'>".$su->salespersoncode.": ".$su->name."</option>";
                $usedList[] = $code;
            }
        ...
        <div class="form-group">
            <label for="salespersonnew"><b>Vælg ny sælger [<?php echo $this->adminOrderHelper->getLanguageCode(); ?>]:</b></label><br>
    ...
    public function deliveryoptions($companyorderid=0,$orderhash="")
    {
        ...
        <p><b>VIGTIGT:</b> en spærring blokkere for godkendelse af nye valg. Er der allerede lavet valg som er godkendt og klar til levering vil de ikke blive annulleret. Se herunder om der er nogle leverancer der er godkendt. Kontakt teknisk support hvis de skal fjernes for ikke at blive sendt ud.</p>
        
        <div class="alert alert-info" role="alert">
            <?php

                $shipments = \Shipment::find_by_sql("select * from shipment where companyorder_id = " . $order->id . " and shipment_type not in ('giftcard','earlyorder')");
                if(count($shipments) > 0) {
                    echo "Der er ".count($shipments)." godkendte eller sendte gavevalg, de vil ikke være påvirket af en spærring.";
                } else {
                    echo "Der er pt. ingen godkendte gavevalg klar til at blive sendt ud.";
                }
