 ...
 */
    ...
    async LoadOrderItems(orderID){
        ...
        if(i.item[0].type == "BONUSPRESENTS" && orderData.data.result[0].order_state > 3) disabled = "disabled";
        ...
        if(window.USERID == 86 || window.USERID == 63 || window.USERID == 50 || window.USERID == 110 || window.USERID == 138 || window.USERID == 338 || window.USERID == 155 || window.USERID == 144 || window.USERID == 124 ){
    ...
    console.log('IS ADMIN: '+(this.isAdmin ? 'YES' : 'NO'));
        console.log(orderData);

        // If itemRules.extra.card_values is set, not null and != ""
        if(itemRules.hasOwnProperty('extra') && itemRules.extra.hasOwnProperty('card_values') && itemRules.extra.card_values != "") {

            try {
            var selectedValues = orderData.data.result[0].card_values;
            var selectedValuesArray = selectedValues.split(',');
            
            var valueString = itemRules.extra.card_values;
            var values = valueString.split(',');

            $("#OrderEditValuesContainer").show();
            $("#OrderEditValues").html(tpOrderlist.ValuesFormShops(values,selectedValues));

            } catch(e) {
                console.log(e);
                $("#OrderEditValuesContainer").hide();
                $("#OrderEditValues").html('');
            }
    ...
    async save(orderID){
        ...
        if($('#sendob').is(':checked')) {
            orderdata.sendob = 1;
        }
        
        if(window.USERID == 86 || window.USERID == 63 || window.USERID == 50 || window.USERID == 110 || window.USERID == 138 || window.USERID == 338 || window.USERID == 155 || window.USERID == 144){
        ...
        var card_values = [];
        $('.cardvalues:checked').each(function() {
            card_values.push($(this).val());
        });
        orderdata.card_values = card_values.join(',');
