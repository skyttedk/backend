...
// længde :
    ...
    // skal ikke med i listen: 1840
    $(document).ready(function() {

        $('#autopilot').on('click', async function() {
    ...
    async function listHandler() {
        for (let i = 0; i < shopList.length; i++) {
            ...
            _adapt = 0;
            shopID = shopList[i];
            $("#autostats").html("<<"+ shopID +" | "+(i+1)+":"+ shopList.length+">>")
            await loadShopData(shopID);
            await sleep(1000);
            await shopCompleted();
            await sleep(1000);
            await autopilot(shopID);
            await sleep(1000);
            await doSave(shopID)
            await sleep(3000);

        }
        alert("Alle shops er behandlet");
    }
    ...
    function doSave(shop_id){
        return new Promise(function(resolve, reject) {

        $.ajax({
            url: APPR_AJAX_URL + 'updateReservationer',
            method: 'POST',
            data:{shop_id:shop_id,data:_newResavation,adapt:_adapt},
            dataType: 'json',
            success: function(data) {
    ...
    function autopilot(shop_id){
        return new Promise(function(resolve, reject) {
        $.ajax({
            url: APPR_AJAX_URL + 'autopilot',
            method: 'POST',
            dataType: 'json',
            data:{shop_id:shop_id,total_orders:_totalOrders,procent_selected:_procentSelected},
            success: function(data) {
                if(data.status == 0){
