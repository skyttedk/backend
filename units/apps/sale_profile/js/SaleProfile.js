var SALEPROFILE_AJAX_URL = "https://system.gavefabrikken.dk/gavefabrikken_backend/index.php?rt=unit/apps/sale_profile/";
import Base from 'https://system.gavefabrikken.dk/gavefabrikken_backend/units/cardshop/main/js/base.js';

export default class SaleProfile extends Base {
    constructor() {
        super();
        this.profiles = [];
        this.editMode = false;
    ...
    async init(langId = 1) {
        try {
            ...
            console.log("SaleProfile initialized successfully");
        } catch (error) {
    ...
    renderProfileTable() {
        ...
        $('#saleProfileContainer').html(html);
    ...
    makeRequest(action, data = {}) {
        return new Promise((resolve, reject) => {
            $.post(SALEPROFILE_AJAX_URL + action, data, function(response) {
                try {
    ...
    makeFormRequest(action, formData) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: SALEPROFILE_AJAX_URL + action,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
