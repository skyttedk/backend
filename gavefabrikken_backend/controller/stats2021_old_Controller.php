...
class stats2021Controller
{
    ...
    public function cardSale()
    {
        ...
        $data_2022_dk = unserialize(base64_decode($this->getOldData2022(1, 2022)));
        $data_2022_no = unserialize(base64_decode($this->getOldData2022(4, 2022)));
        $data_2022_se = unserialize(base64_decode($this->getOldData2022(5, 2022)));
        ...
        $data_2023_dk = array(
            "total" => $this->getSalePresentYear(1),
...
// ----------------------- 2020 -----------------
        ...
        $data_2023_no = array(
            "total" => $this->getSalePresentYear(4),
...
// ----------------------- 2020 -----------------
        ...
        $sortData = $this->buildCol2021($data_2020_dk, "total", 4);
        ...
        $data_2023_se = array(
            "total" => $this->getSalePresentYear(5),
...
// ----------------------- 2020 -----------------
        ...
        $sortData = $this->buildCol2021($data_2020_dk, "total", 5);
    ...
    public function getOldData2022($lang, $year = 2022)
    {
    ...
    public function buildCol2021($data, $coll, $lang)
    {
        ...
        $langListe = [];
        if ($lang == 1) {
            $langListe = array(52, 4668, 53, 2395, 54, 55, 56, 290, 310, 575, 4662, 2548, 2961, 2960, 2962, 2963);
        }
        if ($lang == 4) {
            $langListe = array(272, 57, 58, 59, 574, 2550, 2549, 4740);
        }
        if ($lang == 5) {
            $langListe = array(1832, 1981, 2558, 4793, 5117);
        }
    ...
    public function getSalePresentYear($lang, $start = "2023-01-01 16:47:14", $slut = "2023-12-24 23:47:14")
    {
        $sql = "SELECT
                language_code,
            ...
            LEFT JOIN(
                ...
                WHERE
                    company_order.`created_datetime` > '" . $start . "' AND company_order.`created_datetime` <= '" . $slut . "'  and `salesperson` not like ('%us%') and company_order.order_state not IN(7,8,20)  AND company_order.is_cancelled = 0  AND company_order_id NOT IN( select id from company_order where company_id in(  44780,44794,44795,45363,45364,45365,52468,52469,52470))
  ...
  location.reload();
    ...
    public function getAllStats()
    {
        ...
        if (isset($dg2022[$shopId])) {
            $result = $dg2022[$shopId];
        } else {
            $result = "ingen data";
        }
        ...
        if ($_POST["shop_id"] == "0") {
            ...
            if ($_POST["alias"] == 0) {
                echo "<label><b>Total DG 2022: " . $result . "</b></label>";
                // $this->loadStats_org($_POST["deadline"], $_POST["shop_id"]);
    ...
    public function loadStats($deadline,$shopId,$returnCsv=false)
    {
        ...
        if ($shopId == "0") {
            ...
            $sqlNotSelect = "SELECT COUNT(*) as antal  FROM `shop_user` WHERE 
                                                ...
                                                company_order_id not in(SELECT id FROM company_order where is_cancelled = 1 || order_state  IN(7,8,20)   ) AND 
                                                company_order_id NOT IN( select id from company_order where company_id in(   44780 ,44794,44795,45363,45364,45365  )) AND
                                                company_order_id in( select id from company_order where company_order.`created_datetime` > '2023-01-01 16:47:14' AND company_order.`created_datetime` <= '2023-12-24 23:47:14' )  AND
                                                blocked = 0 and blocked = 0  and is_demo = 0 " . $expireDateSql . " order by antal";
        }
...
company_order_id not in(SELECT id FROM company_order where is_cancelled = 1 || order_state  IN(7,8,20)   ) and
company_order_id NOT IN( select id from company_order where company_id in(   44780 ,44794,44795,45363,45364,45365,52468,52469,52470  )) and  
...
GROUP by present_model.model_present_no order by present_model.model_present_no, present_model.model_name,present_model.model_no
            ...
            $sql = "
            SELECT present_model.model_name,  present_model.model_no, present_model.model_present_no,COUNT(`present_model_id`) as antal, navision_item.standard_cost,cardshop_settings.card_price,  present_reservation.quantity, present_model.active as model_active   FROM `order`
inner JOIN present_model on present_model.model_id = `order`.`present_model_id`
inner JOIN shop_user on shop_user.id = `order`.`shopuser_id`
...
GROUP by present_model.model_present_no order by present_model.model_present_no, present_model.model_name,present_model.model_no";
        ...
        foreach ($rs as $dataRow) {
            ...
            $seShops = array("1832","1981","2558","4793","5117");
        ...
        $html .= "  </tbody></table>";
        $html .= "<div class='css-db " . $cssRed . "'><table ><tr><td>Total DB</td><td>" . $totalSale . "</td></tr><tr><td>Total DG %</td><td>" . $totalDBCalcProcent . " %</td></tr></table> </div>";
...
//52,53,54,55,290,310,575,2548,2395
    ...
    public function dbCalcV2SE(){
        $this->dbCalcV2("1832,1981,2558,4793,5117",1);
    }
...
SELECT
            ...
            present_model.model_no,
            present_model.model_name,
            COUNT(`order`.id) as order_antal,
           present_reservation.quantity

            quantity
            FROM `present_model`
                inner JOIN present on `present_model`.`present_id` = present.id
                LEFT join `order`  on `order`.`present_model_id` = present_model.model_id
                left join present_reservation on present_reservation.model_id = present_model.model_id
                left join shop_present on shop_present.present_id = present_model.present_id
                ".$deadlineSql."

                where
                    present_model.language_id = 1 AND
                    present.`shop_id` = ".$shopID." and
                    present_reservation.shop_id = ".$shopID."
                    GROUP by present_model.model_id
                ORDER BY `order_antal` ASC";
 */
?>
