...
class cardOrderMailsController extends baseController
{
    ...
    public function csMailAfterSalePhysical()
    {
        ...
        switch ($lang) {
            case 1:
                $this->mailAfterSalePhysical($id);
                break;
            case 4:
                $this->mailAfterSalePhysicalNorge($id);
                break;
            case 5:
                $this->mailAfterSalePhysicalSverige($id);
                break;
            default:
                echo "none";
                return;
    ...
    public function mailAfterSalePhysical($companyOrderID = "")
    {
        ...
        $this->sendWelcomeMailPhysicsCardForOrder($companyOrderID);
    }

    public function mailAfterSalePhysicalNorge($companyOrderID = "")
    {
        // Sanitize incoming id (may come from POST indirectly)
        $companyOrderID = $this->sanitizeId($companyOrderID);
        if ($companyOrderID === null) {
        ...
        $this->sendWelcomeMailPhysicsCardNorgeForOrder($companyOrderID);
    }

    public function mailAfterSalePhysicalSverige($companyOrderID = "")
    {
        // Sanitize incoming id (may come from POST indirectly)
        $companyOrderID = $this->sanitizeId($companyOrderID);
        if ($companyOrderID === null) {
        ...
        $this->sendWelcomeMailPhysicsCardSverigeForOrder($companyOrderID);
    }
    ...
    private function sendWelcomeMailPhysicsCardSverige()
    {
        ...
        $listOfNewOrders = Companyorder::all(array('conditions' => array(
            "is_email = 0 and order_state = 5 and welcome_mail_is_send = 0 and send_welcome_mail = 1 and shop_id in (select shop_id from cardshop_settings where language_code = 5 and send_certificates = 1)  and welcome_check = 0 and created_datetime < NOW() - INTERVAL 5 DAY limit 1"
        )));
        ...
        if (sizeofgf($listOfNewOrders) > 0) {
            foreach ($listOfNewOrders as $order) {
                if ($this->hasReceivedWelcomeMail($order->company_id)) {
                    ...
                    $mailTemplate = mailtemplate::find(29);
                    $mailTemplate->template_receipt;
                    $template = str_replace('{token}', $token, $mailTemplate->template_receipt);
                    $maildata = [];
    ...
    {
        $listOfNewOrders = Companyorder::all(array('conditions' => array(
            "is_email = 0 and order_state = 5 and welcome_mail_is_send = 0 and send_welcome_mail = 1 and shop_id in (select shop_id from cardshop_settings where language_code = 4 and send_certificates = 1)  and welcome_check = 0 and created_datetime < NOW() - INTERVAL 5 DAY limit 1"
        )));
    ...
    // Methods to send welcome mails for specific orders (for resending functionality)
    private function sendWelcomeMailPhysicsCardForOrder($companyOrderID)
    {
        $order = Companyorder::find($companyOrderID);
        if (!$order || $order->is_email == 1 || $order->is_cancelled == 1) {
            echo json_encode(array("status" => 0, "message" => "Invalid order or not physical card"));
            return;
        }
    ...
    private function sendWelcomeMailPhysicsCardNorgeForOrder($companyOrderID)
    {
        ...
        $recipent = $order->contact_email;
        if ((int)$companyOrderID === 11111167870) {
    ...
    private function sendWelcomeMailPhysicsCardSverigeForOrder($companyOrderID)
    {
        $order = Companyorder::find($companyOrderID);
        if (!$order || $order->is_email == 1 || $order->is_cancelled == 1) {
            echo json_encode(array("status" => 0, "message" => "Invalid order or not physical card"));
