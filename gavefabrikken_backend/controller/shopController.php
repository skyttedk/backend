...
class ShopController Extends baseController {
  ...
  private function validatetodkenShop($shopID,$token)
  {
    ...
    public function setbackground(){
        ...
        $sql = "update shop set login_background = ".$id." where id=".$shopID;
  ...
  public function setSold(){
      $amount = $_POST["amount"];
      $shopID = $_POST["shop_id"];
      $sql = "update shop set sold = ".$amount." where id=".$shopID;
      $res = Dbsqli::setSql2($sql);
      response::success($res);
  }
public function getSold(){
    $shopID = $_POST["shop_id"];
    $sql = "select sold from shop where id=".$shopID;
...
public function copy(){
    ...
    $name =  $shop->attributes["name"]."-copy-".time();
    $link =  $shop->attributes["link"]."-".time();
    $pt_shopName = $shop->attributes["pt_shopname"]."-".time();
    $shopData = json_encode(array(
        "name"=>$name,
        "link"=>$link,
        "pt_shopName"=>$pt_shopName
    ));
    // Company
    $companyShop = CompanyShop::find_by_shop_id(6121);
    $company_id =   $companyShop->attributes["company_id"];
    $company = Company::find($company_id);
    $company = $company->attributes;
    $companyData = json_encode(array(
        "name"=>$name,
        "cvr"=>$company["cvr"],
        "username"=>"admin",
    ...
    // Descriptions
    $options = array('shop_id' => $shop_id);
    $descriptions = ShopDescription::find('all', $options);
    $descriptionsData = [];
    foreach ($descriptions as $p){
        $descriptionsData[] = json_encode($p->attributes);
    };
    ...
    $newShop = shop::createCompanyShop($postData);
    print_R($newShop);
    ...
    public function createCompanyShop() {
        ...
        $newShopID = $shop->id;
        $shopMetadata = ShopMetadata::find_by_shop_id($newShopID);
        $companyData = (array) json_decode($_POST['company']);
        $shopMetadata->salesperson_code = strtoupper($companyData["sales_person"]);
        $shopMetadata->save();

        response::success(make_json("shop", $shop));
    }
    public function generatePassword($length = 6) {
        // Definerer det tegnsæt, der skal bruges (a-z og 0-9)
        $characters = 'abcdefghijklmnopqrstuvwxyz0123456789';
        $charactersLength = strlen($characters);
        $password = '';

        // Vælg tilfældigt et tegn fra $characters for hver iteration
        for ($i = 0; $i < $length; $i++) {
  ...
  public function update(){
      if(isset($_POST["final_finished"])){
          if($_POST["final_finished"] == 1){
              $this->closeReservation($_POST);
            }
      }
  ...
  public function closeReservation($postData)
  {


      $id =  $postData["id"];
      $options = array('shop_id' => $id);
      $pr = PresentReservation::find('all', $options);
      $jsonData =  make_json("shop", $pr);
      $presentreservationcloselog = new Presentreservationcloselog();
      $presentreservationcloselog->log = $jsonData;
      $presentreservationcloselog->shop_id = $id;
      $presentreservationcloselog->save();
      System::connection()->commit();

      // tjekker om data er der
      $options = array('shop_id' => $id);
      $prcheck = Presentreservationcloselog::find('all', $options);
      if(sizeof($prcheck)  >0){
          $return = ["status"=>"1"];
          $sql = "UPDATE `present_reservation` set `quantity` = 0, `warning_level` = 0 WHERE `shop_id` = ".$id;
          Dbsqli::setSql2($sql);
      } else {
          $return = ["status"=>"0"];
      }
  ...
  public function checkShopSettings(){
   ...
   private function syncItemsNo($shopID){
    ...
    public function readwarehouses(){
       $warehouse =  NavisionLocation::all(array('show_in_shop' => 1));
        response::success(make_json("warehouse", $warehouse));
    }
  ...
  public function read() {
    ...
    $presentCategory = shopPresentCategory::find_by_shop_id($_POST['id']);
    $allCategories = ShopPresentCategory::find_all_by_shop_id_and_active($_POST['id'],1);
    $shop->attributes["shopPresentCategory"] = $allCategories;
...
// ---------------
    public function readFull_v2_dev() {
        ...
        */
        ...
        if($_POST["id"] == "7121") {
            ...
            $cardValues = explode(",", $responseData["data"]["shop"][0]["card_value"]);
...
*/
     ...
     $shop = shop::readShop($_POST['id']);
       ...
       //  $shop->attributes["link"] = "";
        ...
        // Standard budget shops - indsæt id'er på de shops der har budget på shop_user
        $shopBudgetList = ["7423", "6527", "8826","8897","9718","9911"];
        if (isset($_POST["id"]) && in_array($_POST["id"], $shopBudgetList)) {

            // Find card values på shop user
            if($shopuser->card_values != null) {

                $cardValues = explode(",", $shopuser->card_values);

                // Begræns gaveudvalget til gaver der matcher kortværdierne
                foreach ($responseData["data"]["shop"][0]["presents"] as $key => $present) {
                    if (!in_array($present["present"]["attributes"]["present_list"], $cardValues)) {
                        unset($responseData["data"]["shop"][0]["presents"][$key]);
                    }
  ...
  public function readSimpleByID() {

        $id = $_POST['shopID'];
        $shop = Shop::find($id);
        $link = "";
        if($shop) {
            $link = $shop->attributes["link"];
        }
  ...
  public function readSimple() {
    ...
    if(count($shop)==1)
    {
      ...
      $result['show_tree_front'] = $shop->show_tree_front;
      $result['localisation'] = $shop->localisation;
      $result['login_background'] = $shop->login_background;
  ...
  public function getToUsersHowHasNotPickedUpPresents() {
    ...
    public function getShopAttExt() {
        $this->validatetodkenShop($_POST['id'],$_POST['token']);
  ...
  public function getShopPresentsExt() {


        $shop_id = $_POST["shop_id"];
        $this->validatetodkenShop($shop_id,$_POST["token"]);
        $shop =  Shop::find_by_sql("select * from shop where  id = ".$shop_id." and shop_mode = 2");

        if(sizeof($shop) > 0 ){

       //     $this->syncItemsNo($shop_id);
        }
        $presents = PresentModel::find_by_sql(" select * from present_model where
            present_id in ( SELECT id FROM `present` WHERE `shop_id` = ".intval($shop_id)."   and deleted = 0 and id
		in( SELECT present_id FROM `shop_present` WHERE `shop_id` = ".intval($shop_id)."  AND `is_deleted` = 0 and active = 1 )
  ...
  public function readAll() {
    ...
    public function readAllShopsAndCardshops() {
  ...
  public function getSoftCloseOld(){
    ...
    */
    ...
    public function getSoftClose(){
        $shop_id = $_POST['shop_id'];
        if(!is_numeric($shop_id)) return;
        $rs = Shop::find_by_sql("SELECT id, soft_close,edit_allowed,close_date FROM shop WHERE id = ".$shop_id );
        ...
        $response = array(
            ...
            "data" => array(
                "shops" => array(
                    0 => array(
                        "soft_close" => $rs[0]->soft_close,
                        "edit_allowed" => $rs[0]->edit_allowed,
                        "close_date"=>$rs[0]->close_date
                    )
                )
            )
        );
  ...
  public function searchCompanyShopsSimple() {
    ...
    if(isset($_POST["all"])){
      ...
      $isSO = substr($what, 0, 2);
      $isSO = strtolower($isSO);
      if (strtolower($isSO) === 'so' ) {
          $rs = Company::find_by_sql("SELECT shop_id FROM `company_shop` WHERE `company_id` in (SELECT id FROM `company` WHERE `so_no` LIKE '%$what%')");

          if(sizeof($rs) > 0){
              $shopID =  $rs[0]->attributes["shop_id"];
              $shops = Shop::find('all', array(
                  'order' => 'name asc',
                  'conditions' => array("deleted = 0 AND id = $shopID")
              ));
              $options = array('only' => array('id', 'name'));
              response::success(make_json("shops", $shops, $options));
              return;
  ...
  public function updateCompanyShop() {
    ...
    $companyData = (array) json_decode($_POST['company']);
    $shopData = (array) json_decode($_POST['shop']);

    $sqlMeta = "update shop_metadata set so_no =  '".$companyData["so_no"]."' where shop_id = ".$shopData["id"];
    Dbsqli::setSql2($sqlMeta);

    $sqlShop = "update shop set manager_code = '".strtoupper($companyData["gift_responsible"])."' where id = ".$shopData["id"];
    Dbsqli::setSql2($sqlShop);
...
//   ***************   present function *****
  ...
  // User functions
    public function kundepanelremoveUser()
    {
...
/*
  ...
  */
    ...
    public function kundepanelAddSUser() {

        $data = $_POST;
        $jdata = json_decode($_POST['data'], true);
        $this->validatetodkenShop($jdata['shopId'],$jdata['token']);
 ...
 //   $shopuser->shop_id    = $toshop->id;
  ...
  //Model functions
   ...
   //kundepanel_email_regel
    ...
    public function setKundepanelEmailRegel() {
       $this->validatetodkenShop($_POST['shop_id'],$_POST['token']);
   ...
   // Luk shop så der kan udleveres gaver
    public function   closeForRegistration() {
       $this->validatetodkenShop($_POST['shop_id'],$_POST['token']);
        $shop = Shop::find($_POST['shop_id']);
     ...
     public function setQrMenuSettings() {
        $this->validatetodkenShop($_POST['shop_id'],$_POST['token']);
        $shop = Shop::find($_POST['shop_id']);
   ...
   // check om der kan udleveres gaver
    public function kundepanelSettings() {
        $this->validatetodkenShop($_POST['shop_id'],$_POST['token']);
  ...
  public function loadfreightdata() {

      $id = $_POST["id"];
      $shopmetadata = \ShopMetadata::find_by_sql("select * from shop_metadata where shop_id = ".intvalgf($id));
      if(isset($shopmetadata[0])) {
          echo json_encode(array("use_dot" => $shopmetadata[0]->dot_use,"use_carryup" => $shopmetadata[0]->carryup_use));
      } else {
          echo json_encode(array("use_dot" => 0,"use_carryup" => 0));
      }
