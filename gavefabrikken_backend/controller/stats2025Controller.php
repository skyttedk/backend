...
class stats2021Controller
{
    ...
    public function cardSale()
    {
        ...
        //  [start] => 2022-09-11 00:00:01 [end] => 2023-09-10 19:31:39 )
        //00 [antal] => 96 ) [13] => Array ( [shop_id] => 2961 [language_code] => 1 [concept_code] => LUKS-200 [antal] => 207 ) ) [s] => 2022-09-01 00:00:01 [e] => 2022-09-30 23:59:59 ) asdf
...
// ----------------------- 2020 -----------------
    ...
    public function buildCol2021($data, $coll, $lang)
    {
        ...
        if ($lang == 5) {
            $langListe = array(4793,1832,1832440, 5117,1981, 2558,8271 );
        }
    ...
    public function loadStats($deadline,$shopId,$returnCsv=false)
    {
        ...
        if ($shopId == "0") {
            $sqlNotSelect = "SELECT COUNT(*) as antal  FROM `shop_user` WHERE 
                                               ...
                                               company_order_id NOT IN( select id from company_order where company_id in(   44780 ,44794,44795,45363,45364,45365,44780,69437,69439,69451,
                    44794,
                    44795,
                    45363,
                    45364,
...
44780,69437,69439,69451
                                                ...
                                                company_order_id in( select id from company_order where company_order.`created_datetime` > '2024-01-01 16:47:14' AND company_order.`created_datetime` <= '2024-12-24 23:47:14' )  AND
...
company_order_id not in(SELECT id FROM company_order where is_cancelled = 1 || order_state  IN(7,8,20)   ) and
company_order_id NOT IN( select id from company_order where company_id in(   44780 ,44794,44795,45363,45364,45365,52468,52469,52470,                    44780,69437,69439,69451,
                    44794,
                    44795,
                    45363,
                    45364,
...
//52,53,54,55,290,310,575,2548,2395
    ...
    public function dbCalcV2DK(){
        //
        $this->dbCalcV2("52,53,2395,9321,54,55,56,575,2548,290,310",1);
    }
    public function dbCalcV2NO(){
        $this->dbCalcV2("272,57,58,59,574,2550,2549,4740",4);
    }
    public function dbCalcV2SE(){
        $this->dbCalcV2("1832,1981,2558,4793,5117,8271",1);
    }
