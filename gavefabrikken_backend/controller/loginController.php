...
class LoginController Extends baseController {
  ...
  public function Index() {
      ...
      //if(!isset($_GET["letme"]) || $_GET["letme"] != "in") die("Vi er ved at opdatere serveren. Vi forventer, at være færdige senest kl. 9.00");
  ...
  {
      unset($_SESSION["syslogin".GFConfig::SALES_SEASON]);
    
        unset($_SESSION["systemuser_login".GFConfig::SALES_SEASON]);
        unset($_SESSION["systemuser_token".GFConfig::SALES_SEASON]);
      $dummy['link']  = '';
  ...
  public function apiLogin(){
    ...
    public function apiLogin2(){

        $shopuser = ShopUser::find_by_username_and_shop_id(strtolower($_POST['username']), $_POST['shop_id']);
        $pw = $shopuser->password;

        if(strtolower($pw) !=  strtolower($_POST['password'])){
                $dummy["orderStatus"] = "error";
                response::success(json_encode($dummy));
                return;
        }
...
public function tokenLogin(){
    ...
    public function makeNewUser(){
        ...
        $companyToken = $_POST['company_token'];

        $randomToken = bin2hex(openssl_random_pseudo_bytes(16));
        if($companyToken == "04xoZSBPiQyXkHiVBrwvlHMPrJCT3PncthGrG83y"){
            $companyId = "69188";
        }
        ...
        $form["data"] = '{"userId":null,"shopId":"'.$shopid.'","companyId":"'.$companyId.'"}';
  ...
  public function loginShopUser() {
      ...
      $shopsWithSingleUserLogin = array(8681,8493,9114,9534);
      ...
      if(in_array(intval($_POST['shop_id']),$shopsWithSingleUserLogin) && $_POST['username'] != "admin") {
          ...
          $token = ShopUser::Login($_POST['shop_id'],$random,$random);
          $shopUserNewLogin = ShopUser::getByToken($token);
          ActionLog::logShopUserAction("ShopUserLogin", $shopUserNewLogin->username." er logget ind","",$shopUserNewLogin);
          $options = array('except' => array('password'));
          response::success(make_json("result", $shopUserNewLogin,$options));
