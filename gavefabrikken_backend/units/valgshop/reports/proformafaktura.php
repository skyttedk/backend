...
class ProformaFaktura
{
    ...
    public function showDownloadForm() {
        ...
        <div style="padding: 10px;">
            ...
            <div style="padding: 10px;">
                Tilføj pallevægt:<br>
                Antal helpaller (25 kg): <input type="text" name="pallet_amount" size="6" value=""> stk<br>
                Antal halvpaller (12 kg): <input type="text" name="half_pallet_amount" size="6" value=""> stk<br>
            </div>
...
*/
        ...
        foreach($vareNrMap as $varenr => $count) {
            ...
            if(count($navisionItemList) > 0) {
                ...
                if(floatval($weightNet) > 0) {
                    $weightGross = $weightNet * 1.2;
                }
            ...
            $sheet->setCellValueByColumnAndRow(4, $row, "=F".$row."*1.2");
    ...
    private function loadNavInvoiceData($soNo,$debitorNo) {
        ...
        try {
            ...
            if($result == null) {
                ...
                $customerClient = new CustomerWS(1);
                $customer = $customerClient->getByCustomerNo($debitorNo);
                if($customer == null) {
                    echo "NO SALES ORDER FOUND: " . $soNo;
                ...
                return array(
                    ...
                    "zipcity" => $customer->getPostCode()." ".$customer->getCity(),
                    "country" => $customer->getCountryCode(),
                    "phone" => $customer->getPhone(),
                    "att" => $customer->getContact(),
                    "no" => $debitorNo,
                    "orderno" => $soNo
                );
    ...
    public function downloadFakturaV2() {
        ...
        if(count($companyShop) > 0) {
            ...
            $invoiceData = $this->loadNavInvoiceData($shopMetadata->so_no,$shopMetadata->nav_debitor_no);
            if($invoiceData === null) {
    ...
    private function writeSheetV2($name,$vareNrMap,$amount,$adress,$invoiceData,$invoiceNo=1,$vatno="",$soNo="") {
        ...
        foreach($vareNrMap as $varenr => $count) {
            ...
            $sheet->setCellValueByColumnAndRow(7, $row, "=F".$row."*1.2");
            $sheet->setCellValueByColumnAndRow(8, $row, $count);
            $sheet->setCellValueByColumnAndRow(9, $row, $amount);
...
// Anvend formatet på kolonne F og H for det pågældende rækkeområde
        ...
        $palletAmount = intvalgf($_GET["pallet_amount"] ?? 0);
        $halfPalletAmount = intvalgf($_GET["half_pallet_amount"] ?? 0);

        $sheet->setCellValue("B19", "Gross weight total (KG)");
        $sheet->setCellValue("B20", "=ROUND((SUM(G".$rowStart.":G".$rowEnd.")+(".$halfPalletAmount."*12+".$palletAmount."*25)), 2)");
        ...
        $sheet->setCellValue("E".$row+1, "=B20");
        ...
        $row += 5;
        $sheet->setCellValue("C".$row,"GaveFabrikken, Carl Jacobsens vej 16, 2500 Valby, Denmark");
