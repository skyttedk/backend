...
export default class Cardsoptions extends Base {
    ...
    async Layout(){
       ...
       if(this.isCompany == "valgshopReplacement"){
                  $("#tabsMasterData-3").html("" +
                      " <button class='delete-replacementCard' data-id='"+this.shopuserID+"'>Slet kortet (fx hvis kunden skal have et nyt erstatsningskort) </button> " +
    ...
    async SetUpdateMasterdataEvent(){
        ...
        $(".removeMasterData").unbind("click").click(
            function(){
                self.onRemoveShopuserOrder(self.shopuserID);
            }
        )
    ...
    async onRemoveShopuserOrder(shopuserID) {

        if(confirm('Slettes ordren vil ordre og ordrenr fjernes fra systemet og gavekortet åbnes op og kunne vælges på igen. Der sendes ikke besked til kunden ved sletning af ordren. Sletning vil fejl hvis ordren har været sendt ud eller der er nogle komplikationer ift. ordren som gør at den ikke kan slettes. Bemærk at brugerens informationer ikke nulstilles, nulstil manuelt herover hvis en anden skal have gavekortet. Overvej at bruge erstatningskort i stedet. Er du sikker på at du vil slette ordren?')) {

            let response = await super.post("cardshop/cards/removeorder/",{shopuserid: shopuserID});
            if(response.status == 1) {
                alert('Ordren er slettet. Kunden skal genindlæses for at det kan ses i gavekort listen.');
                $('.modal').modal('hide');
            } else if(response.error) alert(response.error);
            else alert('Der skete en fejl. Ordren er ikke slettet.');
    ...
    async deleteReplacementCard(shop_id){
        let r = confirm("Er du sikke på du vil slette kortet");
        if(r){
            await super.post("cardshop/cards/deleteReplacementCard/"+shop_id);
            $('.modal').modal('hide');
            $('.companylist-selected').trigger('click');
        }
