...
class AdminOrderHelper
{
    ...
    public function __construct($companyorderid,$requireHash=false,$orderhash = "")
    {
        ...
        $adminUsers = array(50,86,110,138,199,153,147,124,162,51,66,190,145,217,286,285,305,304,291,338,331);
    ...
    public function canChangeFloatingDeadline() {
        return $this->co->floating_expire_date != null;
    }
    ...
    public function reopenCompanyOrder($endBuffer=true)
    {
        ...
        // Opdater shipments
        $shipmentList = \Shipment::find("all",array("conditions" => array("shipment_type" => "giftcard","companyorder_id" => $order->id)));
        if(count($shipmentList) > 0) {
            foreach($shipmentList as $shipment) {
                ...
                $shipment->shipment_note = $shipment->shipment_note." Genåbnet ordre, kort kan være sendt! ";
                $shipment->save();
            }
        }
