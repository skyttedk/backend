...
*/
    ...
    async OrderFormDeadline(shopType){
        ...
        $("#OrderFormDeadlineSelect").unbind("change").change(function() {
            ...
            ME.OrderFormShippingMethod( {allinclusive: $(this).find(":selected").attr("allinclusive"), expire_date:$(this).val(),sale_is_open: $(this).find(":selected").attr("saleisopen"), special:$(this).find(":selected").attr("special"), websale_is_open: $(this).find(":selected").attr("websaleisopen") } );
    ...
    async OrderFormShippingMethod(options) {
        ...
        if(options.hasOwnProperty('allinclusive') && options.allinclusive == "true") {
            // sleet 500 ms
            setTimeout(function() {
                $('#OrderFormPhysicalShippingInput').prop('checked',true).trigger('change').trigger('click')
            }, 500);
