            ...
            <div class="p-1 flex-fill">
                ...
                <select class="form-control selectpicker mb-2" multiple data-live-search="true" id="deadline">
                    <?php foreach($expireDates as $expireDate) {
                        echo '<option value="'.$expireDate->expire_date->format("Y-m-d").'">'.$expireDate->expire_date->format("Y-m-d").' - '.($expireDate->is_delivery == 1 ? "privatlevering" : 'Uge '.$expireDate->week_no).' '.($expireDate->is_floating == 1 ? "(flydende deadline)" : "").'</option>';
                    } ?>
                </select>
            </div>
            <div class="p-1 flex-fill">
