...
class EarlyOrderModel
{
    ...
    public function listProvider($isCSV=false)
    {
        ...
        // Shipmentstatus
        $shipmentStatus = $data["shipmentStatus"] ?? array();

        if(count($shipmentStatus) > 0) {

            $shipmentStateSQLList = [];
            foreach($shipmentStatus as $shipmentState)
            {
                $shipmentState = intval($shipmentState);
                if($shipmentState > 0) {
                    $shipmentStateSQLList[] = $shipmentState;
                }
            }

            if(count($shipmentStateSQLList) > 0) {
                $searchSQL .= " and s.shipment_state IN (".implode(",",$shipmentStateSQLList).") ";
            }
        ...
        // Payment status
        $paymentStatus = $data["paymentStatus"] ?? array();
        if(count($paymentStatus) > 0) {

            $paymentStateSQLList = [];
            foreach($paymentStatus as $paymentState)
            {

                $paymentState = intval($paymentState);
                if($paymentState == 10) {
                    $paymentStateSQLList[] = "co.prepayment = 0";
                } else if($paymentState == 20) {
                    $paymentStateSQLList[] = "(co.prepayment = 1 && co.nav_paid = 0)";
                } else if($paymentState == 30) {
                    $paymentStateSQLList[] = "(co.prepayment = 1 && co.nav_paid = 1)";
                }

            }

            if(count($paymentStateSQLList) > 0) {
                $searchSQL .= " and (".implode(" || ",$paymentStateSQLList).") ";
            }
 ...
 //       exit();
        ...
        if($isCSV) {
            ...
            $headers = [
                ...
                'Leverancestatus','Ordrestatus','Betalingsstatus', 'Samleordre', 'Linjer', 'Oprettet', 'Modtager', 'Adresse1', 'Adresse 2',
            ...
            <tr>
                ...
                <th data-sortfield="paystate">Betalingsstatus</th>
    ...
    public function getOrderListCSVRow($order,$output,$separator)
    {
        ...
        $payState = "Ukendt";
        if($order->prepayment == 0) $payState = "Ingen aconto";
        else if($order->prepayment == 1 && $order->nav_paid == 1) $payState = "Betalt aconto";
        else if($order->prepayment == 1 && $order->nav_paid == 0) $payState = "Ikke betalt aconto";
    ...
    public function getOrderListRow($order)
    {
        ...
        <td><?php echo htmlspecialcharsgf($payState); ?></td>
