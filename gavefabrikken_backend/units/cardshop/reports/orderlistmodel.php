...
class OrderListModel
{
    ...
    LEFT JOIN company_shipping_cost csc ON c.id = csc.company_id
WHERE co.shop_id in (".implode(",",$this->shopIDList).") ".$searchSQL." ".$sortSQL;
        ...
        if($isCSV) {
            ...
            // Define the headers
            $headers = [
                'ID', 'BS nr', 'Virksomhed', 'CVR', 'Sælger', 'Koncept','Beløb', 'Antal', 'Deadline', 'Type',
    ...
    <tr>
        ...
        <th data-sortfield="freight">Fragt</th>
    </tr>
    </thead>
        <tbody><?php
    ...
    public function getOrderListCSVRow($order,$output,$separator)
    {
        ...
        if($order->is_delivery == 1) {
            $freightVal = "Privatlevering";
        } else if($order->manual_freight == 1) {
            $freightVal = "Skal beregnes manuelt";
        }
        else if($order->freight_cost !== null) {
            $freightVal = "Fastpris: ".$order->freight_cost;
        }
        else {
            $freightVal = "Brug matrix";
        }
        ...
        ?><tr>
            ...
            <td><?php
                ...
                if($order->is_delivery == 1) {
                    ...
                    echo "Skal beregnes manuelt";
                }
                else if($order->freight_cost !== null) {
                    echo "Fastpris: ".$order->freight_cost;
                }
                else {
                    echo "Brug matrix";
                }
