...
";
    ...
    public function loadTotal(){
       ...
       $sql = "SELECT count(shop_user.id) as antal,shop_user.shop_id from (SELECT id,shop_name FROM `company_order` WHERE is_cancelled = 0 AND company_order.company_id NOT IN(
                    44780,
                    44794,
                    44795,
                    45363,
                    45364,
                    45365) AND company_order.order_state not IN(7,8)  and `salesperson` not like ('%us%') and shop_id in(".$this->getCountryId($country).") ".$rangeSql."  ) as co
                        inner join shop_user ON shop_user.company_order_id = co.id and
                        shop_user.shop_id in( ".$this->getCountryId($_POST["country"])." ) and
                        is_demo = 0 and
                        shop_user.blocked = 0 group by shop_user.shop_id order by co.shop_name";
        $rs =  order::find_by_sql($sql);
        response::success(make_json("soldcard", $rs));
    }
    public function loadDBSaleperson(){
       ...
       $sql = "SELECT  COUNT(DISTINCT(co.order_no)) as orderAntal, count(shop_user.id) as antal, co.shop_id, co.salesperson, shop_user.shop_id from (SELECT order_no, shop_name ,id,`salesperson`,shop_id FROM `company_order` WHERE `salesperson` not like ('%us%') and is_cancelled = 0 AND company_order.company_id NOT IN(
                    44780,
                    44794,
                    44795,
                    45363,
                    45364,
                    45365)  and shop_id  in(  ".$this->getCountryId($country).") ".$rangeSql." ) as co
                        inner join shop_user ON shop_user.company_order_id = co.id and
                        shop_user.shop_id  in(  ".$this->getCountryId($_POST["country"]).") and
                        is_demo = 0 and
...
/*------------------valgshop  --------------------------*/
    ...
    {
        $countryIdList["dk"] = "'52','53','54','55','56','290','310','575','2548','2395','2960','2962','2963','2961','2999'";
        $countryIdList["no"] = "'57','58','59','272','574','2549','2550'";
        $countryIdList["se"] = "'1832','1981','2558'";
