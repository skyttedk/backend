...
// Created by Bitworks
class newshoploadController Extends baseController {
    ...
    public function getUsers()
    {
        ...
        $totalUser = ShopUser::find_by_sql("SELECT count(username) as antal FROM shop_user where shop_id = ".$shopid);
//        $shopusers = ShopUser::find_by_sql("SELECT *,CONVERT(username USING utf8) as  from shop_user where shop_id = ".$shopid." ORDER BY username COLLATE utf8_danish_ci ;" );

        $shopusers = ShopUser::all(
            array(
            'order' => 'username',
            'conditions' => array('shop_id' => $shopid,'is_demo' => 0),
            'limit' => $_POST['limit'],
            'offset' =>  $_POST['offset']
            ));
        ...
        $response = array("status" => 1,"message" => "", "data" => array("users" => $responseList),"antal"=>$totalUser);
