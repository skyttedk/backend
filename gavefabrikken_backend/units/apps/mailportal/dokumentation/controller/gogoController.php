...
';
        ...
        if($deadline == "03-01-2023" || $deadline == "07-11-2022"){
          $mailTemplate = mailtemplate::find(10);
        } else {
          $mailTemplate = mailtemplate::find(8);
        }
...
';
        ...
        if($shop_id == "2960" || $shop_id == "2961" || $shop_id == "2962" || $shop_id == "2963" || $shop_id == "2999"){
            ...
            if($deadline == "01-04-2023" || $deadline == "31-12-2023"){
  ...
  private function getWebCardNorge(){
     ...
     select id from company_order WHERE
        ...
        shop_id in ( 57,58,59,272,574,2550,2549) limit 1");
  ...
  private function getWebCardSverige(){
     ...
     select id from company_order WHERE
        ...
        order_state in (4,5) and
         is_email = 1  and
        created_datetime < NOW() - INTERVAL 2 HOUR  and
        shop_id in ( 1832,1981,2558) limit 1");
    // echo $ordernr[0]["attributes"]["id"];
    if(sizeofgf($rs) == 0){
        return false;
    } else {
  ...
  private function getWebCard(){
     ...
     select id from company_order WHERE
        ...
        is_cancelled = 0 and
         is_email = 1  and
        created_datetime < NOW() - INTERVAL 2 HOUR  and
        (shop_id in (select shop_id from cardshop_settings where language_code = 1) && shop_id IN (SELECT shop_id FROM `cardshop_settings` WHERE `language_code` = 1) || company_id = 45396) limit 1");
    // echo $ordernr[0]["attributes"]["id"];
    if(sizeofgf($rs) == 0){
        return false;
    } else {
