...
class AppController Extends baseController
{
    ...
    public function index($arg1="",$arg2="")
    {
        ...
        array_shift($rtSplit);
        $unitFolder = array_shift($rtSplit);
        $unitName = array_shift($rtSplit);
        if($unitName == "") $unitName = "index";

        // Check if exists
        $className = "GFApp\\".$unitFolder."\\".$unitName."\\Controller";
        if(class_exists($className) == false) {
            echo "App does not exist: ".$className;
            return;
        }

        // Get action
        $actionName = array_shift($rtSplit);
        if(trimgf($actionName) == "") $actionName = "view";
        ...
        if(!method_exists($unitController,$actionName)) {
            ...
            echo "Invalid app path: ".$actionName;
            return;
        }
