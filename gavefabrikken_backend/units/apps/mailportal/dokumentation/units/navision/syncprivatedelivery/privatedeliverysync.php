...
class PrivateDeliverySync
{
    ...
    private $seSpaHotel = array('200325','200326','200327','sam1950','200327','sam1950'); // Important, keep letters in lowercase
    ...
    public function __construct($output = false)
    {
        ...
        $this->replacementCardCompanyList = array(44780,52468,45363,52469,52470);
    ...
    private function runPrivateDelivery($shopUserID,$OrderID)
    {
        ...
        try {
            ...
            if(($shipment->handler == "glsexport" || $shipment->handler == "mydsv" || $shipment->handler == "navision") && $isSESpaHotel) {
            ...
            $shipment->shipto_phone = trim(utf8_encode($userData["telefon"]));
        ...
        try {
            ...
            if(trimgf($e->getMessage()) == "Invalid se postal code, not 5 digits") {
                $shipment->shipment_state = 9;
            } else {
