    ...
    public function mapPostfields($deployStoreView,$konData,$kon,$online,$showPrice,$country){
      //  $showPrice = true;
        ...
        if($country == 4){
            ...
            // temp pris
/*
            $priseRs = \Dbsqli::getSql2("select * from temp_no_budget where itemno ='".$itemno."'");
            if (sizeof($priseRs) == 0) {
                die("ingen no pris");
            }
            $price = $priseRs[0]["budget"];
*/
 ...
 */
        ...
        if($storeview == "Gaveklubben" && $showPrice == 1){
        ...
        $result =  $this->updateWebsiteIds($itemno,$list);
    ...
    public function updateWebsiteIds($sku,$storeViewsArray){
        $websiteIds = [];
        $MagentoApi = new magentoapi();
...
//        $this->updateProductWebsite0($postData);
        ...
        $url = 'https://gavefabrikken.dk/rest/'.$deployStoreView.'/V1/products/';
