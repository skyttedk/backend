        ...
        foreach($freightItems as $freighItem) {
            ...
            try {
                ...
                if($freightObject->dot == 1) {
                    ...
                    try {
                        $freightObject->dot_date = new \DateTime($data['dotdate']);
                        if ($freightObject->dot_date->format('Y') < \GFConfig::SALES_SEASON || $freightObject->dot_date->format('Y') > \GFConfig::SALES_SEASON) {
                            throw new \Exception("Årstal ikke korrekt.");
                        }
                    } catch (\Exception $e) {
                        $this->addError("Der er ikke angivet en gyldig dato til dot", $freighItem->getUniqueKey());
                    }
