...
<div style="position: fixed; top: 200px; bottom: 90px; left: 0px; width: 100%; ">
    <div class="scrollable-table" style="width: 100%; height: 100%; overflow: auto;">
        ...
        <table class="table table-bordered" id="orderlisttable" style="font-size: 0.8em;"></table>
...
<div style="position: fixed; top: 0px; left: 0px; padding: 15px; background: #F5F5F5; width: 100%; height: 200px;">
    <div class="row sticky-top">
        ...
        <div class="col-12 mt-3 d-flex flex-wrap">
            ...
            <div class="p-1 flex-fill">
                Deadline:<br>
                <select class="form-control selectpicker mb-2" multiple data-live-search="true" id="deadline">
                    <?php foreach($expireDates as $expireDate) {
                        echo '<option value="'.$expireDate->expire_date->format("Y-m-d").'">'.$expireDate->expire_date->format("Y-m-d").' - '.($expireDate->is_delivery == 1 ? "privatlevering" : 'Uge '.$expireDate->week_no).' '.($expireDate->expire_date->format("Y-m-d") == "2030-01-01" ? "(flydende deadline)" : "").'</option>';
                    } ?>
                </select>
            </div>
            <div class="p-1 flex-fill">
                Sælger:<br>
                <select class="form-control selectpicker mb-2" multiple data-live-search="true" id="seller">
                    <?php foreach($salespersons as $salesPerson) {
                        echo '<option value="'.$salesPerson.'">'.$salesPerson.'</option>';
                    } ?>
                </select>
            </div>
            <div class="p-1 flex-fill">
                Ordre status:<br>
                <select class="form-control selectpicker mb-2" multiple data-live-search="true" id="order-status">
                    <?php foreach($orderStateList as $value => $orderState) {
...
<script>
    ...
    function updateTable() {
        ...
        $.ajax({
            url: '<?php echo $servicePath; ?>orderlistprovider',
            type: 'POST',
            ...
            success: function(response) {
                ...
                $('#orderlisttable').html(data.content);
                $('.loading-overlay').hide();

                $('#orderlisttable th').bind('click',function(e) {
                    updateSort(e)
                })

                // Find data-sortfield=sortField and add class sorted sort+sortDir
                $('#orderlisttable th').removeClass('sorted sortedasc sorteddesc');
                $('#orderlisttable th[data-sortfield="'+sortField+'"]').addClass('sorted sorted'+sortDir);
    ...
    function downloadAsCSV() {
        var searchData = collectSearchCriteria();
        window.location.href = '<?php echo $servicePath; ?>orderlistcsv&'+$.param(searchData);
    }
