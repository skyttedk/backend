...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <form class="form-inline my-2 my-lg-0">
        ...
        <button class="btn btn-outline-success my-2 my-sm-0" type="button" onclick="updateReservationSearchNav2()">Nav liste v2.</button>
        &nbsp;
...
<script>
    ...
    function updateReservationSearchNav2() {
        var itemno = $('input[name="itemno"]').val();
        var location = $('select[name="location"]').val();
        var languageid = $('select[name="languageid"]').val();
        $.ajax({
            url: '<?php echo $servicePath; ?>reservationsearchnav2',
            type: 'POST',
            data: {
                itemno: itemno,
                location: location,
