...
class nusoap_xmlschema extends nusoap_base  {
	...
	*/
	...
	function CreateTypeName($ename) {
		$scope = '';
		for ($i = 0; $i < countgf($this->complexTypeStack); $i++) {
	...
	function serializeSchema(){
		...
		foreach($this->complexTypes as $typeName => $attrs){
			...
			if (isset($attrs['restrictionBase']) && $attrs['restrictionBase'] != ''){
				...
				if ((isset($attrs['elements']) && countgf($attrs['elements']) > 0) || (isset($attrs['attrs']) && countgf($attrs['attrs']) > 0)){
		...
		// simple types
		if(isset($this->simpleTypes) && countgf($this->simpleTypes) > 0){
		...
		// elements
		if(isset($this->elements) && countgf($this->elements) > 0){
			foreach($this->elements as $element => $eParts){
				$xml .= " <$schemaPrefix:element name=\"$element\" type=\"".$this->contractQName($eParts['type'])."\"/>\n";
			}
		}
		// attributes
		if(isset($this->attributes) && countgf($this->attributes) > 0){
	...
	function getResponse(){
	  ...
	  if ($this->io_method() == 'socket') {
	    ...
	    while (!isset($lb)){
			...
			$tmplen = strlen($tmp);
			$this->debug("read line of $tmplen bytes: " . trimgf($tmp));
		...
		// process headers
		$header_data = trimgf(substr($data,0,$pos));
		...
		foreach($header_array as $header_line){
			...
			if(count($arr) > 1){
				$header_name = strtolower(trimgf($arr[0]));
				$this->incoming_headers[$header_name] = trimgf($arr[1]);
				if ($header_name == 'set-cookie') {
					// TODO: allow multiple cookies from parseCookie
					$cookie = $this->parseCookie(trimgf($arr[1]));
					if ($cookie) {
						$this->incoming_cookies[] = $cookie;
						$this->debug('found cookie: ' . $cookie['name'] . ' = ' . $cookie['value']);
					} else {
						$this->debug('did not find cookie in ' . trimgf($arr[1]));
					}
    			}
		...
		do {
			if ($chunked) {
				...
				$content_length = hexdec(trimgf($tmp));
...
//		}
		...
		$http_reason = countgf($arr) > 2 ? $arr[2] : '';
 		...
 		if (isset($this->incoming_headers['www-authenticate']) && $http_status == 401) {
 			...
 			if (strstr($this->incoming_headers['www-authenticate'], "Digest ")) {
 				...
 				foreach ($digestElements as $val) {
 					$tempElement = explode('=', trimgf($val), 2);
	...
	*/
	function serialize_return() {
		...
		// returned value(s)
			...
			if($this->wsdl){
				if (sizeofgf($this->opData['output']['parts']) > 1) {
					$this->debug('more than one output part, so use the method return unchanged');
			    	$opParams = $this->methodreturn;
			    } elseif (sizeofgf($this->opData['output']['parts']) == 1) {
...
class wsdl extends nusoap_base {
		...
		switch ($name) {
			case 'import':
			    if (isset($attrs['location'])) {
                    ...
                    $this->debug('parsing import ' . $attrs['namespace']. ' - ' . $attrs['location'] . ' (' . countgf($this->import[$attrs['namespace']]).')');
				} else {
                    $this->import[$attrs['namespace']][] = array('location' => '', 'loaded' => true);
					if (! $this->getPrefixFromNamespace($attrs['namespace'])) {
						$this->namespaces['ns'.(count($this->namespaces)+1)] = $attrs['namespace'];
					}
                    $this->debug('parsing import ' . $attrs['namespace']. ' - [no location] (' . countgf($this->import[$attrs['namespace']]).')');
				}
				break;
			//wait for schema
			//case 'types':
    ...
    */
	function getTypeDef($type, $ns) {
		...
		if (isset($this->schemas[$ns])) {
			...
			for ($i = 0; $i < countgf($this->schemas[$ns]); $i++) {
	...
	*/
	...
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
		...
		// set input params
		$xml = '';
		if (isset($opData[$direction]['parts']) && sizeofgf($opData[$direction]['parts']) > 0) {
			$parts = &$opData[$direction]['parts'];
			$part_count = sizeofgf($parts);
			$style = $opData['style'];
			$use = $opData[$direction]['use'];
			$this->debug("have $part_count part(s) to serialize using $style/$use");
			if (is_array($parameters)) {
				$parametersArrayType = $this->isArraySimpleOrStruct($parameters);
				$parameter_count = countgf($parameters);
	...
	function serializeParameters($operation, $direction, $parameters)
	{
		...
		if (isset($opData[$direction]['parts']) && sizeofgf($opData[$direction]['parts']) > 0) {
			...
			$use = $opData[$direction]['use'];
			$this->debug("use=$use");
			$this->debug('got ' . countgf($opData[$direction]['parts']) . ' part(s)');
			if (is_array($parameters)) {
				$parametersArrayType = $this->isArraySimpleOrStruct($parameters);
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		if ($phpType == 'struct') {
			...
			if (is_array($value) && sizeofgf($value) >= 1) {
				$rows = sizeofgf($value);
				$contents = '';
				foreach($value as $k => $v) {
	...
	function serializeComplexTypeElements($typeDef, $value, $ns, $uqType, $use='encoded', $encodingStyle=false) {
		...
		if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
			...
			if (count($typeDef['elements']) != countgf($xvalue)){
				$optionals = true;
			}
	...
	function end_element($parser, $name) {
		...
		if(isset($this->body_position) && $pos > $this->body_position){
			...
			// build complexType values of attributes and possibly simpleContent
				...
				} elseif (isset($this->message[$pos]['cdata']) && trimgf($this->message[$pos]['cdata']) != '') {
	...
	*/
	function buildVal($pos){
		...
		if($this->message[$pos]['children'] != ''){
			...
			if (isset($this->message[$pos]['cdata']) && trimgf($this->message[$pos]['cdata']) != '') {
...
//var_dump(array($return['faultcode'],$return['faultstring']));
				$this->setError($return['faultcode'].': '.print_r($return['faultstring'],true));
				$this->fault = true;
	...
	*/
	function checkWSDL() {
		...
		if ($errstr = $this->wsdl->getError()) {
			...
			$this->debug('got '.countgf($this->operations).' operations from wsdl '.$this->wsdlFile.' for binding type '.$this->bindingType);
	...
	function _getProxyClassCode($r) {
		...
		foreach ($this->operations as $operation => $opData) {
			if ($operation != '') {
				...
				if (sizeofgf($opData['input']['parts']) > 0) {
					$paramStr = '';
					$paramArrayStr = '';
					$paramCommentStr = '';
    ...
    */
	...
	function checkCookies() {
		if (sizeofgf($this->cookies) == 0) {
			return true;
		}
		$this->debug('checkCookie: check ' . sizeofgf($this->cookies) . ' cookies');
		...
		$this->debug('checkCookie: '.sizeofgf($this->cookies).' cookies left in array');
		return true;
	}
	...
	function UpdateCookies($cookies) {
		...
		foreach ($cookies as $newCookie) {
			...
			$found = false;
			for ($i = 0; $i < countgf($this->cookies); $i++) {
