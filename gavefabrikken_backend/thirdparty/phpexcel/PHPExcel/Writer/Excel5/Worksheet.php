...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
		...
		$this->_countCellStyleXfs = countgf($phpSheet->getParent()->getCellStyleXfCollection());
	}
	...
	function close()
	{
		...
		if (!empty($this->_colinfo)) {
			$colcount = countgf($this->_colinfo);
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	private function _writeMergedCells()
	{
		...
		$countMergeCells = countgf($mergeCells);
	...
	private function _writeBreaks()
	{
		...
		if (!empty($hbreaks)) {
			...
			$cbrk	= countgf($hbreaks);	   // Number of page breaks
		...
		if (!empty($vbreaks)) {
			...
			$cbrk	= countgf($vbreaks);	   // Number of page breaks
			$length  = 2 + 6 * $cbrk;	  // Bytes to follow
	...
	private function _writeMsoDrawing()
	{
		...
		if (isset($this->_escher)) {
			...
			$spOffsets[0] = 0;
			$nm = countgf($spOffsets) - 1; // number of shapes excluding first shape
			for ($i = 1; $i <= $nm; ++$i) {
				// MSODRAWING record
	...
	private function _writeDataValidity()
	{
		...
		if (!empty($dataValidationCollection)) {
			...
			$data		= pack('vVVVV', $grbit, $horPos, $verPos, $objId,
										 countgf($dataValidationCollection));
			$this->_append($header.$data);
	...
	private function _writeCFRule(PHPExcel_Style_Conditional $conditional){
		...
		$numConditions = sizeofgf($arrConditions);
		if($numConditions == 1){
	...
	private function _writeCFHeader(){
		...
		$header	  = pack('vv', $record, $length);
		$data	  = pack('vv', countgf($arrConditional), $needRedraw);
		$data     .= $cellRange;
		$data     .= pack('v', 0x0001);
		$data     .= $cellRange;
		$this->_append($header . $data);
