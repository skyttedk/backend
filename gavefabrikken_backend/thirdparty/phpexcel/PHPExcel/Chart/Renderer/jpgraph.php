...
class PHPExcel_Chart_Renderer_jpgraph
{
	...
	private function _formatPointMarker($seriesPlot,$markerID) {
		...
		if (is_null($markerID)) {
			...
			self::$_plotMark %= countgf(self::$_markSet);
	...
	private function _renderPlotLine($groupID, $filled = false, $combination = false, $dimensions = '2d') {
        ...
        $labelCount = countgf($this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotValuesByIndex(0)->getPointCount());
		if ($labelCount > 0) {
	...
	private function _renderPlotStock($groupID) {
		...
		for($j = 0; $j < countgf($dataValues[0]); $j++) {
			for($i = 0; $i < $seriesCount; $i++) {
		...
		for($groupID = 0; $groupID < $iLimit; ++$groupID) {
			...
			for($j = 0; $j < $jLimit; ++$j) {
				...
				if (count($datasetLabels) > 0)
					$seriesPlot->SetLabels(array_fill(0,countgf($datasetLabels),''));
				if ($dimensions != '3d') {
