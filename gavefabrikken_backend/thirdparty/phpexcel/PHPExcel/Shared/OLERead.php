	...
	private function _readPropertySets() {
		...
		while ($offset < $entryLen) {
			...
			if (($upName === 'WORKBOOK') || ($upName === 'BOOK')) {
				$this->wrkbook = countgf($this->props) - 1;
			}
			else if ( $upName === 'ROOT ENTRY' || $upName === 'R') {
				// Root entry
				$this->rootentry = countgf($this->props) - 1;
			}

			// Summary information
			if ($name == chr(5) . 'SummaryInformation') {
//				echo 'Summary Information<br />';
				$this->summaryInformation = countgf($this->props) - 1;
			}

			// Additional Document Summary information
			if ($name == chr(5) . 'DocumentSummaryInformation') {
//				echo 'Document Summary Information<br />';
				$this->documentSummaryInformation = countgf($this->props) - 1;
			}
