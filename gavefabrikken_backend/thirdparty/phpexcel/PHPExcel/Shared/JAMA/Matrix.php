...
class PHPExcel_Shared_JAMA_Matrix {
	...
	public function __construct() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				...
				case 'array':
						$this->m = countgf($args[0]);
						$this->n = countgf($args[0][0]);
						$this->A = $args[0];
						break;
				//Square matrix - n x n
				case 'integer':
				...
				case 'array,integer':
						$this->m = $args[1];
						if ($this->m != 0) {
							$this->n = countgf($args[0]) / $this->m;
						} else {
							$this->n = 0;
						}
						if (($this->m * $this->n) == countgf($args[0])) {
							for($i = 0; $i < $this->m; ++$i) {
								for($j = 0; $j < $this->n; ++$j) {
	...
	public function getMatrix() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				...
				case 'array,array':
						list($RL, $CL) = $args;
						if (count($RL) > 0) { $m = countgf($RL); } else { throw new PHPExcel_Calculation_Exception(self::ArgumentBoundsException); }
						if (count($CL) > 0) { $n = countgf($CL); } else { throw new PHPExcel_Calculation_Exception(self::ArgumentBoundsException); }
						$R = new PHPExcel_Shared_JAMA_Matrix($m, $n);
						for($i = 0; $i < $m; ++$i) {
							for($j = 0; $j < $n; ++$j) {
	...
	public function concat() {
		if (func_num_args() > 0) {
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) {
					$this->A[$i][$j] = trimgf($this->A[$i][$j],'"').trimgf($M->get($i, $j),'"');
				}
			}
			return $this;
		} else {
