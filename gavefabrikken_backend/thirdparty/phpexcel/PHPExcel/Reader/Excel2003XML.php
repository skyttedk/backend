			...
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				switch((string) $propertyAttributes) {
					case 'string' :
						...
						$propertyValue = trimgf($propertyValue);
						break;
					case 'boolean' :
					...
					case 'dateTime.tz' :
						...
						$propertyValue = strtotime(trimgf($propertyValue));
						break;
				}
									...
									foreach($temp as &$value) {
										...
										if ($key = !$key) {
											...
											foreach($cellReferences as $cellReference) {
												...
												if ($rowReference{0} == '[') $rowReference = $rowID + trimgf($rowReference,'[]');
												$columnReference = $cellReference[4][0];
												//	Empty C reference is the current column
												if ($columnReference == '') $columnReference = $columnNumber;
												//	Bracketed C references are relative to the current column
												if ($columnReference{0} == '[') $columnReference = $columnNumber + trimgf($columnReference,'[]');
