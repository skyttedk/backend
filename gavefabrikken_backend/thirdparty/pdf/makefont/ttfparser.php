...
/*		$chars = array_keys($this->chars);
	...
	function AddGlyph($id)
	{
		...
		{
			$this->glyphs[$id]['ssid'] = countgf($this->subsettedGlyphs);
	...
	{
		$this->LoadTable('hhea');
		$numberOfHMetrics = countgf($this->subsettedGlyphs);
	...
	{
		$this->LoadTable('maxp');
		$numGlyphs = countgf($this->subsettedGlyphs);
	...
	function BuildPost()
	{
		...
		{
			// Version 2.0
			$numberOfGlyphs = countgf($this->subsettedGlyphs);
			$numNames = 0;
			$names = '';
