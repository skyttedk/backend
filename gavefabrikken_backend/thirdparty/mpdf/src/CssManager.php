...
class CssManager
{
	...
	function ReadCSS($html)
	{
		...
		for ($i = 0; $i < countgf($m[0]); $i++) {
			if ($this->mpdf->CSSselectMedia && !preg_match('/(' . trimgf($this->mpdf->CSSselectMedia) . '|all)/i', $m[1][$i])) {
		...
		if ($CSSstr) {
			...
			for ($i = 0; $i < countgf($styles[1]); $i++) {
				// SET array e.g. $classproperties['COLOR'] = '#ffffff';
				$stylestr = trimgf($styles[2][$i]);
				...
				$tagstr = strtoupper(trimgf($styles[1][$i]));
				...
				foreach ($tagarr as $tg) {
					...
					$tags = preg_split('/\s+/', trimgf($tg));
					$level = countgf($tags);
					$t = '';
					$t2 = '';
					$t3 = '';
					if (trimgf($tags[0]) == '@PAGE') {
						if (isset($tags[0])) {
							$t = trimgf($tags[0]);
						}
						if (isset($tags[1])) {
...
//print_r($this->cascadeCSS); exit;
	...
	function readInlineCSS($html)
	{
		...
		$classproperties = [];
		for ($i = 0; $i < countgf($properties); $i++) {
			// Ignores -webkit-gradient so doesn't override -moz-
			...
			$classproperties[strtoupper($properties[$i])] = trimgf($values[$i]);
		}
		...
		foreach ($prop as $k => $v) {
			...
			if ($k == 'FONT') {
				...
				if (preg_match('/(lower-roman|upper-roman|lower-latin|lower-alpha|upper-latin|upper-alpha|decimal|disc|circle|square|arabic-indic|bengali|devanagari|gujarati|gurmukhi|kannada|malayalam|oriya|persian|tamil|telugu|thai|urdu|cambodian|khmer|lao|cjk-decimal|hebrew)/i', $v, $m)) {
					$newprop['LIST-STYLE-TYPE'] = strtolower(trimgf($m[1]));
				} else if (preg_match('/U\+([a-fA-F0-9]+)/i', $v, $m)) {
					$newprop['LIST-STYLE-TYPE'] = strtolower(trimgf($m[1]));
				}
				if (preg_match('/url\([\'\"]{0,1}(.*?)[\'\"]{0,1}\)/i', $v, $m)) {
					$newprop['LIST-STYLE-IMAGE'] = strtolower(trimgf($m[1]));
				}
				if (preg_match('/(inside|outside)/i', $v, $m)) {
					$newprop['LIST-STYLE-POSITION'] = strtolower(trimgf($m[1]));
				}
			} else {
				$newprop[$k] = $v;
			}
	...
	function setCSSboxshadow($v)
	{
		...
		foreach ($ss as $s) {
			...
			$p = explode(' ', trimgf($s));
			if (isset($p[0])) {
				$new['x'] = $this->sizeConverter->convert(trimgf($p[0]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
			}
			if (isset($p[1])) {
				$new['y'] = $this->sizeConverter->convert(trimgf($p[1]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
			}
			if (isset($p[2])) {
				if (preg_match('/^\s*[\.\-0-9]/', $p[2])) {
					$new['blur'] = $this->sizeConverter->convert(trimgf($p[2]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
				} else {
					$new['col'] = $this->colorConverter->convert(preg_replace('/\*/', ',', $p[2]), $this->mpdf->PDFAXwarnings);
				}
				if (isset($p[3])) {
					if (preg_match('/^\s*[\.\-0-9]/', $p[3])) {
						$new['spread'] = $this->sizeConverter->convert(trimgf($p[3]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
					} else {
	...
	function setCSStextshadow($v)
	{
		...
		foreach ($ss as $s) {
			...
			$p = explode(' ', trimgf($s));
			if (isset($p[0])) {
				$new['x'] = $this->sizeConverter->convert(trimgf($p[0]), $this->mpdf->FontSize, $this->mpdf->FontSize, false);
			}
			if (isset($p[1])) {
				$new['y'] = $this->sizeConverter->convert(trimgf($p[1]), $this->mpdf->FontSize, $this->mpdf->FontSize, false);
			}
			if (isset($p[2])) {
				if (preg_match('/^\s*[\.\-0-9]/', $p[2])) {
					$new['blur'] = $this->sizeConverter->convert(trimgf($p[2]), $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, false);
				} else {
					$new['col'] = $this->colorConverter->convert(preg_replace('/\*/', ',', $p[2]), $this->mpdf->PDFAXwarnings);
				}
				if (isset($p[3])) {
	...
	function border_radius_expand($val, $k)
	{
		$b = [];
		if ($k == 'BORDER-RADIUS') {
			$hv = explode('/', trimgf($val));
			$prop = preg_split('/\s+/', trimgf($hv[0]));
			if (count($prop) == 1) {
		...
		if ($tag == 'TR' && isset($p) && $p) {
			foreach ($p as $k => $val) {
				if (preg_match('/' . $tag . '>>SELECTORNTHCHILD>>(.*)/', $k, $m)) {
					...
					if ($tag == 'TR') {
						$row = $this->mpdf->row;
						$thnr = (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_thead']) ? countgf($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_thead']) : 0);
						$tfnr = (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot']) ? countgf($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot']) : 0);
		...
		foreach (['TOP', 'RIGHT', 'BOTTOM', 'LEFT'] as $side) {
			foreach (['STYLE', 'WIDTH', 'COLOR'] as $el) {
				if (isset($a['BORDER-' . $side . '-' . $el])) { // e.g. $b['BORDER-TOP-STYLE']
					$s = trimgf($a['BORDER-' . $side . '-' . $el]);
					if (isset($b['BORDER-' . $side])) { // e.g. $b['BORDER-TOP']
						$p = trimgf($b['BORDER-' . $side]);
					} else {
						$p = '';
					}
					if ($el == 'STYLE') {
