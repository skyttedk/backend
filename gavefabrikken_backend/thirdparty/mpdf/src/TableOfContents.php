...
class TableOfContents
{
	...
	public function insertTOC()
	{
		...
		for ($toci = 0; $toci < $notocs; $toci++) {
			...
			// mPDF 5.6.31
			$tocstart = countgf($this->mpdf->pages);
		...
		if ($extrapage) {
			unset($this->mpdf->pages[countgf($this->mpdf->pages)]);
	...
	public function openTagTOCPAGEBREAK($attr)
	{
		...
		if ($this->mpdf->y == $this->mpdf->tMargin && (!$this->mpdf->mirrorMargins || ($this->mpdf->mirrorMargins && $this->mpdf->page % 2 == 1))) {
			...
			// Don't add a page
			if ($this->mpdf->page == 1 && countgf($this->mpdf->PageNumSubstitutions) == 0) {
				$resetpagenum = '';
				$pagenumstyle = '';
				$suppress = '';
