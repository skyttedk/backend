...
class Svg
{
	...
	{
		$n = countgf($this->mpdf->gradients) + 1;
	...
	function xml_svg2pdf_start($parser, $name, $attribs)
	{
		...
		switch (strtolower($name)) {
			...
			case 'text':
				...
				$this->textanchor = $this->txt_style[countgf($this->txt_style) - 1]['text-anchor']; // mPDF 5.7.4
			...
			case 'tspan':
				...
				if (isset($attribs['x']) || isset($attribs['y'])) {
					...
					$this->textanchor = $this->txt_style[countgf($this->txt_style) - 1]['text-anchor'];
					$this->textXorigin = $x;
					$this->textYorigin = $y;
				} else {
					$this->textXorigin += $this->textlength;
