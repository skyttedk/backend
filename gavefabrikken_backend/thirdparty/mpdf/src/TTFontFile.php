			...
			for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
				...
				if ($Lookup[$i]['Type'] == 1) {
					...
					if ($SubstFormat == 1) {
						...
						$nIsubs = (2 * $nInput) - 1;
						$contextInputMatch = $this->_makeGSUBcontextInputMatch($inputGlyphs, $ignore, [], 0);
						$subRule = ['context' => 1, 'tag' => $tag, 'matchback' => $backtrackMatch, 'match' => ($contextInputMatch . $lookaheadMatch), 'nBacktrack' => countgf($backtrackGlyphs), 'nInput' => $nInput, 'nLookahead' => countgf($lookaheadGlyphs), 'rules' => [],];
