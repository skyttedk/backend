		...
		switch ($tag) {
			...
			case 'FORMFEED': //custom-tag
				...
				if (isset($attr['SHEET-SIZE'])) {
					...
					$prop = preg_split('/\s+/', trimgf($attr['SHEET-SIZE']));
					if (count($prop) == 2) {
			...
			case 'ANNOTATION':
				...
				if (isset($attr['POPUP']) && !empty($attr['POPUP'])) {
					$pop = preg_split('/\s+/', trimgf($attr['POPUP']));
					if (count($pop) > 1) {
						$objattr['POPUP'] = $pop;
					} else {
						$objattr['POPUP'] = true;
			...
			case 'BR':
				...
				if (count($this->mpdf->InlineBDF)) {
					...
					if (count($iBDF)) {
						ksort($iBDF);
						for ($i = countgf($iBDF) - 1; $i >= 0; $i--) {
							$inlinepre .= $this->mpdf->_setBidiCodes('end', $iBDF[$i]);
						}
						for ($i = 0; $i < countgf($iBDF); $i++) {
				...
				if ($this->mpdf->tableLevel) {
					...
					if (count($this->mpdf->textbuffer)) {
						$this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0] = preg_replace('/ $/', '', $this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0]);
						if (!empty($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][18])) {
							$this->mpdf->otl->trimOTLdata($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][18], false, true);
						} // *OTL*
					}
			...
			case 'MAIN':
				...
				if ($this->mpdf->blockjustfinished && !countgf($this->mpdf->textbuffer) && $this->mpdf->y != $this->mpdf->tMargin && $this->mpdf->collapseBlockMargins) {
				...
				if (isset($properties['FLOAT']) && strtoupper($properties['FLOAT']) == 'RIGHT' && !$this->mpdf->ColActive) {
					...
					if (isset($currblk['css_set_width']) && $currblk['css_set_width']) {
...
//		if ($currblk['inner_width'] < $mw) { throw new MpdfException("DIV is too narrow for text to fit!"); }
			...
			case 'BARCODE':
				...
				if (isset($attr['CODE']) && $attr['CODE']) {
					...
					if (isset($attr['TYPE'])) {
						$objattr['btype'] = trimgf(strtoupper($attr['TYPE']));
					} else {
						$objattr['btype'] = 'EAN13';
					} // default
			...
			case 'JPGRAPH':
				...
				if (!is_array($this->mpdf->graphs[$gid]) || countgf($this->mpdf->graphs[$gid]) == 0) {
					break;
				}
				$this->mpdf->ignorefollowingspaces = false;
			...
			case 'TABLE': // TABLE-BEGIN
				...
				} else if (!$this->mpdf->tableLevel && countgf($this->mpdf->textbuffer)) {
					$this->mpdf->printbuffer($this->mpdf->textbuffer, $blockstate);
				}
				...
				if ($this->mpdf->blockjustfinished && !countgf($this->mpdf->textbuffer) && $this->mpdf->y != $this->mpdf->tMargin && $this->mpdf->collapseBlockMargins && $this->mpdf->tableLevel == 1) {
					$lastbottommargin = $this->mpdf->lastblockbottommargin;
				} else {
					$lastbottommargin = 0;
				}
	...
	{ // mPDF 6
		...
		if ($tag == 'FONT' || $tag == 'SPAN' || $tag == 'CODE' || $tag == 'KBD' || $tag == 'SAMP' || $tag == 'TT' || $tag == 'VAR' || $tag == 'INS' || $tag == 'STRONG' || $tag == 'CITE' || $tag == 'SUB' || $tag == 'SUP' || $tag == 'S' || $tag == 'STRIKE' || $tag == 'DEL' || $tag == 'Q' || $tag == 'EM' || $tag == 'B' || $tag == 'I' || $tag == 'U' | $tag == 'SMALL' || $tag == 'BIG' || $tag == 'ACRONYM' || $tag == 'MARK' || $tag == 'TIME' || $tag == 'PROGRESS' || $tag == 'METER' || $tag == 'BDO' || $tag == 'BDI'
			...
			if ($tag == 'PROGRESS' || $tag == 'METER') {
				...
				if (isset($this->mpdf->InlineProperties[$tag]) && countgf($this->mpdf->InlineProperties[$tag])) {
					$tmpProps = array_pop($this->mpdf->InlineProperties[$tag]); // mPDF 5.7.4
					$this->mpdf->restoreInlineProperties($tmpProps);
				}
				if (isset($this->mpdf->InlineAnnots[$tag]) && countgf($this->mpdf->InlineAnnots[$tag])) {  // *ANNOTATIONS*
					$annot = array_pop($this->mpdf->InlineAnnots[$tag]);  // *ANNOTATIONS*
				} // *ANNOTATIONS*
				if (isset($this->mpdf->InlineBDF[$tag]) && countgf($this->mpdf->InlineBDF[$tag])) {  // mPDF 6
		...
		if ($tag == 'P' || $tag == 'DIV' || $tag == 'H1' || $tag == 'H2' || $tag == 'H3' || $tag == 'H4' || $tag == 'H5' || $tag == 'H6' || $tag == 'PRE' || $tag == 'FORM' || $tag == 'ADDRESS' || $tag == 'BLOCKQUOTE' || $tag == 'CENTER' || $tag == 'DT' || $tag == 'DD' || $tag == 'DL' || $tag == 'CAPTION' || $tag == 'FIELDSET' || $tag == 'UL' || $tag == 'OL' || $tag == 'LI' || $tag == 'ARTICLE' || $tag == 'ASIDE' || $tag == 'FIGURE' || $tag == 'FIGCAPTION' || $tag == 'FOOTER' || $tag == 'HEADER' || $tag == 'HGROUP' || $tag == 'MAIN' || $tag == 'NAV' || $tag == 'SECTION' || $tag == 'DETAILS' || $tag == 'SUMMARY'
			...
			if (preg_match('/^H\d/', $tag) && !$this->mpdf->tableLevel && !$this->mpdf->writingToC) {
				if (isset($this->mpdf->h2toc[$tag]) || isset($this->mpdf->h2bookmarks[$tag])) {
					...
					if (count($this->mpdf->textbuffer) == 1) {
						...
						for ($i = 0; $i < countgf($this->mpdf->textbuffer); $i++) {
			...
			if (count($this->mpdf->textbuffer) && $this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1]) {
				if (substr($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0], 0, 3) != "\xbb\xa4\xac") { // not special content
					// Right trim last content and adjust OTLdata
					if (preg_match('/[ ]+$/', $this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0], $m)) {
						$strip = strlen($m[0]);
						$this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0] = substr($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0], 0, (strlen($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][0]) - $strip));
						/* -- OTL -- */
						if (isset($this->mpdf->CurrentFont['useOTL']) && $this->mpdf->CurrentFont['useOTL']) {
							$this->mpdf->otl->trimOTLdata($this->mpdf->textbuffer[countgf($this->mpdf->textbuffer) - 1][18], false, true); // mPDF 6  ZZZ99K
						}
						/* -- END OTL -- */
					}
				}
		...
		if (($tag == 'TH' or $tag == 'TD') && $this->mpdf->tableLevel) {
			...
			if (isset($this->mpdf->cell[$this->mpdf->row][$this->mpdf->col]['textbuffer'])) {
				$ntb = countgf($this->mpdf->cell[$this->mpdf->row][$this->mpdf->col]['textbuffer']);
			} else {
				$ntb = 0;
			}
		...
		if ($tag == 'TABLE') { // TABLE-END (
			...
			if (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot']) && countgf($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot'])) {
				$tfrows = array();
				...
				for ($i = countgf($temp); $i < (count($temp) + countgf($temptf)); $i++) {
					$this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot'][$i] = true;
				}
				// Update nestedpos row references
				if (isset($this->mpdf->table[($this->mpdf->tableLevel + 1)]) && countgf($this->mpdf->table[($this->mpdf->tableLevel + 1)])) {
					foreach ($this->mpdf->table[($this->mpdf->tableLevel + 1)] AS $nid => $nested) {
						$this->mpdf->table[($this->mpdf->tableLevel + 1)][$nid]['nestedpos'][0] -= countgf($temptf);
					}
				}
