...
//print_r($this->cascadeCSS); exit;
	...
	function setCSSboxshadow($v)
	{
		...
		foreach ($ss AS $s) {
			...
			$p = explode(' ', trimgf($s));
			if (isset($p[0])) {
				$new['x'] = $this->mpdf->ConvertSize(trimgf($p[0]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
			}
			if (isset($p[1])) {
				$new['y'] = $this->mpdf->ConvertSize(trimgf($p[1]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
			}
			if (isset($p[2])) {
				if (preg_match('/^\s*[\.\-0-9]/', $p[2])) {
					$new['blur'] = $this->mpdf->ConvertSize(trimgf($p[2]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
				} else {
					$new['col'] = $this->mpdf->ConvertColor(preg_replace('/\*/', ',', $p[2]));
				}
				if (isset($p[3])) {
					if (preg_match('/^\s*[\.\-0-9]/', $p[3])) {
						$new['spread'] = $this->mpdf->ConvertSize(trimgf($p[3]), $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
					} else {
	...
	function setCSStextshadow($v)
	{
		...
		foreach ($ss AS $s) {
			...
			$p = explode(' ', trimgf($s));
			if (isset($p[0])) {
				$new['x'] = $this->mpdf->ConvertSize(trimgf($p[0]), $this->mpdf->FontSize, $this->mpdf->FontSize, false);
			}
			if (isset($p[1])) {
				$new['y'] = $this->mpdf->ConvertSize(trimgf($p[1]), $this->mpdf->FontSize, $this->mpdf->FontSize, false);
			}
			if (isset($p[2])) {
				if (preg_match('/^\s*[\.\-0-9]/', $p[2])) {
					$new['blur'] = $this->mpdf->ConvertSize(trimgf($p[2]), $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, false);
				} else {
					$new['col'] = $this->mpdf->ConvertColor(preg_replace('/\*/', ',', $p[2]));
				}
				if (isset($p[3])) {
