...
class SVG
{
	...
	function svgImage($attribs)
	{
		...
		$orig_srcpath = '';
		if (trimgf($srcpath) != '' && substr($srcpath, 0, 4) == 'var:') {
	...
	{
		$n = countgf($this->mpdf_ref->gradients) + 1;
		...
		if ($element == 'rect') {
			...
			$pts = preg_split('/[ ,]+/', trimgf($attribs['points']));
			$maxr = $maxb = 0;
			$minl = $mint = 999999;
			for ($i = 0; $i < countgf($pts); $i++) {
				if ($i % 2 == 0) { // x values
					$minl = min($minl, $pts[$i]);
					$maxr = max($maxr, $pts[$i]);
				} else { // y values
		...
		if (isset($gradient_info['transform'])) {
			...
			if (count($m[0])) {
				for ($i = 0; $i < countgf($m[0]); $i++) {
					...
					} else if ($c == 'translate' && countgf($vv)) {
						$tm[4] = $vv[0];
						if (count($vv) == 2) {
							$t_y = -$vv[1];
						} else {
		...
		$ns = countgf($gradient_info['color']);
		...
		if ($gradient_info['type'] == 'linear') {
			...
			// Gradient STOPs
			$stops = countgf($gradient_info['color']);
			if ($stops < 2) {
				return '';
			}

			$range = $gradient_info['color'][countgf($gradient_info['color']) - 1]['offset'] - $gradient_info['color'][0]['offset'];
			...
			$grx2 = $x1 + ($x2 - $x1) * $gradient_info['color'][countgf($gradient_info['color']) - 1]['offset'];
			$gry2 = $y1 + ($y2 - $y1) * $gradient_info['color'][countgf($gradient_info['color']) - 1]['offset'];
			...
			$grx2 = $x1 + ($x0 - $x1) * $gradient_info['color'][countgf($gradient_info['color']) - 1]['offset'];
			$gry2 = $y1 + ($y0 - $y1) * $gradient_info['color'][countgf($gradient_info['color']) - 1]['offset'];
			$grir = $r * $gradient_info['color'][0]['offset'];
			$grr = $r * $gradient_info['color'][countgf($gradient_info['color']) - 1]['offset'];
	...
	function svgOffset($attribs)
	{
		...
		if (isset($this->svg_attribs['viewBox'])) {
			$vb = preg_split('/\s+/is', trimgf($this->svg_attribs['viewBox']));
			if (count($vb) == 4) {
...
// If rX = 0 or rY = 0 then this arc is treated as a straight line
	...
	function svgPolyline($arguments, $ispolyline = true)
	{
		...
		for ($i = 2; $i < countgf($arguments); $i += 2) {
			if ($ispolyline) {
				$tmp_x = $arguments[$i];
				$tmp_y = - $arguments[($i + 1)];
			} else {
	...
	function svgText()
	{
		// $tmp = countgf($this->txt_style)-1;
		$current_style = $this->txt_style[countgf($this->txt_style) - 1]; // mPDF 5.7.4
		$style = '';
		$op = '';
		$render = -1;
		...
		$prev_style = $this->txt_style[countgf($this->txt_style) - 1];
		$style = '';
			...
			function xml_svg2pdf_start($parser, $name, $attribs)
			{
				...
				switch (strtolower($name)) {
					...
					case 'path':
						...
						foreach ($commands as $c) {
							if ((isset($c) && countgf($c) == 3) || (isset($c[2]) && $c[2] == '')) {
					...
					case 'text':
						...
						if (_SVG_CLASSES && isset($attribs['class']) && $attribs['class']) {
							$classes = preg_split('/\s+/', trimgf($attribs['class']));
						...
						$svg_class->textanchor = $svg_class->txt_style[countgf($svg_class->txt_style) - 1]['text-anchor']; // mPDF 5.7.4
					...
					case 'tspan':
						...
						$tmp = countgf($svg_class->svg_style) - 1;
						...
						if (isset($attribs['x']) || isset($attribs['y'])) {
							...
							$svg_class->textanchor = $svg_class->txt_style[countgf($svg_class->txt_style) - 1]['text-anchor'];
			...
			if ($i % 2 == 0) {
				...
				for ($sch = 0; $sch <= $subchunk; $sch++) {
					if (isset($chardata[$sch])) {
						$s = '';
						for ($j = 0; $j < countgf($chardata[$sch]); $j++) {
