...
class grad
{
	...
	function CoonsPatchMesh($x, $y, $w, $h, $patch_array = array(), $x_min = 0, $x_max = 1, $y_min = 0, $y_max = 1, $colspace = 'RGB', $return = false)
	{
		...
		for ($i = 0; $i < countgf($patch_array); $i++) {
			...
			for ($j = 0; $j < countgf($patch_array[$i]['colors']); $j++) {
	...
	function Gradient($x, $y, $w, $h, $type, $stops = array(), $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
	{
		...
		if ($repeat) {
			$ns = countgf($this->mpdf->gradients[$n]['stops']);
			$offs = array();
			for ($i = 0; $i < $ns; $i++) {
		...
		if (!is_array($extend) || countgf($extend) < 1) {
	...
	function parseMozGradient($bg)
	{
		...
		if (preg_match('/linear-gradient\((.*)\)/', $bg, $m)) {
			...
			$g['stops'] = array();
			for ($i = $startStops; $i < countgf($bgr); $i++) {
				$stop = array();
				// parse stops
				$el = preg_split('/\s+/', trimgf($bgr[$i]));
				// mPDF 5.3.74
	...
	function parseBackgroundGradient($bg)
	{
		...
		$g = array();
		if (count($bgr) > 6) {
			if (strtoupper(substr($bgr[0], 0, 1)) == 'L' && countgf($bgr) == 7) {  // linear
				$g['type'] = 2;
