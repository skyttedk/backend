...
*/
        ...
        foreach($shops as $shop)
        {
            ...
            */
            ...
            $values[] = array("shopid" => $shop->shop_id, "value" => intval($shop->card_price/100),"min_cards" => $shop->min_web_cards,"bonus_cards" => $bonusCards,"allinclusive" => $shop->allinclusive);
        ...
        $conceptDetails = $this->getConceptDetails($shops[0]);

        if($conceptDetails ["country"] == "NO") {

            foreach($deadlines as $key => $deadline) {
                if($deadline["week"] == 4) {
                    $deadlines[$key]["week"] = 5; // Change week 4 to week 5 for NO
                }
            }
    ...
    private function getShopsByConcept($concept)
    {
        ...
        $shopList = \CardshopSettings::find_by_sql("SELECT * FROM cardshop_settings WHERE concept_parent = '".$concept."' and shop_id not in (8355,8356,8357,8358,8360,8364,8365,8366) ORDER BY card_price ASC");
        $validShopList = array();
    ...
    private function conceptName($concept)
    {
        switch ($concept) {
            ...
            case "SEJKAI":
                return "24 Julklappar - all inclusive";
            default:
