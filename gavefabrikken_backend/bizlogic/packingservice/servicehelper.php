...
class ServiceHelper
{
    ...
    {
        return array(
            "09qC=U5dVBj1jE?61[kqedC&zH^d!@>]_SJAyd6+}Pez#%g;AE"
        );
    }
    ...
    public function authorize()
    {
        ...
        if(!in_array($authHeader,$authList)) {
            $this->outputServiceError(10,"Invalid authorization header: ".$authHeader);
            return false;
        }
    ...
    protected function validLanguage($languageid) {
        $validLanguages = array(1,4,5);
        return in_array(intval($languageid),$validLanguages);
    }
    
    protected function validLocation($location) {
        $location = \NavisionLocation::find('first', array('conditions' => array('code' => $location)));
