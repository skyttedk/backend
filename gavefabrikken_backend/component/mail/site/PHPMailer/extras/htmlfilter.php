...
function tln_tagprint($tagname, $attary, $tagtype)
{
    if ($tagtype == 2) {
        ...
        $fulltag = '<' . $tagname;
        if (is_array($attary) && sizeofgf($attary)) {
            $atts = array();
...
function tln_deent(&$attvalue, $regex, $hex = false)
{
    ...
    if (is_array($matches) && sizeofgf($matches[0]) > 0) {
        $repl = array();
        for ($i = 0; $i < sizeofgf($matches[0]); $i++) {
            $numval = $matches[1][$i];
            if ($hex) {
                ...
                if ($skip_content == false) {
                    ...
                    if ($tagtype == 1
                        ...
                        if (($rm_tags == false
                             ...
                             */
                            if (is_array($attary) && sizeofgf($attary) > 0) {
                                $attary = tln_fixatts(
                                    $tagname,
                                    $attary,
                                    $rm_attnames,
