...
<?php
    ...
    try {
        ...
        foreach($languages as $language) {
            $salesPersonService = new GFCommon\Model\Navision\SalesPersonWS($language);
            $salespersonList = $salesPersonService->getAllSalesPerson();
            foreach($salespersonList as $salesPerson) {
                if(!in_array($salesPerson->getCode(),$usedCodes)) {
                    ...
                    $usedCodes[] = $salesPerson->getCode();
                }
            }
        }

    } catch(Exception $e) {
        $systemUserOptions = "<option value=''> - Fejl, kunne ikke hente fra navision!</option>";
    }
?>
...
<div id="systemUserDialog" style="display:none;">
    <table class = "demo" width=280>
        <tr bgcolor="yellow">
            <td><b>SÆLGERADGANG</b> </td><td width=10><input type="checkbox" class="systemUserAccess" id="tabAccess_1000" data-id="1000" /></td>
        </tr>
        <tr>
