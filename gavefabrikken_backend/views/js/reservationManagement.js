...
var rm = (function () {
    ...
    self.initCardshop = () => {
        ...
        */
        ...
        $("#searchItemPresentationBtn").unbind("click").click(()=> {
            let itemNr = $("#searchItem").val();
            console.log("søg")
            self.searchItemNrInModal(itemNr,"2,4");
        })
    ...
    self.doSearchItemNr = (itemNr,shopStatus=1) => {
        let status = shopStatus;
        console.log(status+"step 2")
        return new Promise(async resolve => {
           $.post(_ajaxPath+"searchItemNr",{itemNr:itemNr,shopStatus:status}, function(data, textStatus) {
                resolve(data);
            },'json');
        });
    }
    ...
    self.searchItemNrInModal = async (itemNr,shopStatus=1) => {
        let status = shopStatus;
        console.log(status+"step 1");
        let self = this;
        $(".modal-body").html("SYSTEMET ARBEJDER")
        $(".modal-title").html("SYSTEMET ARBEJDER");
        self.modalShow();
        let data = await self.doSearchItemNr(itemNr,status);
