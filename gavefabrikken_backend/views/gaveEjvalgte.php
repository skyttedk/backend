...
function opretGaveEjvalgte(gaveid,modelid){
    ...
    if (tjek == "gaver1234" || _ejValgtCounter > 0 || _passpassword == true) {
            ...
            formData["model_id"] = modelid;
                ...
                $.post( "https://system.gavefabrikken.dk/gavefabrikken_backend/index.php?rt=order/autoSelectSpecificPresents",formData, function( data ) {
                    console.log(data)
                    if(data.status == 0){
                        _passpassword == true
                        if(_retry > 10){
                            alert("Jobbet er stoppet, Tryk F5 og proev igen")
                            return;
                        }
                        _retry++;
...
function gaverUnderTildeling(res){
  ...
  if(res.data.orderStatus == 1){
      setTimeout(function (){opretGaveEjvalgte(_ejValgtGaveid,_ejValgtGavemodelid)}, 2000);
