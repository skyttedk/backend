...
/**
 * Escaper XSS-farligt indhold rekursivt
 *
 * @param mixed $input Data der skal escapes
 * @param int $flags HTMLspecialchars flags (valgfri)
 * @param string $encoding Tegnsæt (valgfri)
 * @param int $depth Maksimal rekursionsdybde
 * @return mixed Escaped data
 * @throws InvalidArgumentException Ved for dyb rekursion
 */
function escapexss($input, int $flags = ENT_QUOTES | ENT_HTML5, string $encoding = 'UTF-8', int $depth = 50)
{
    // Beskyttelse mod for dyb rekursion
    if ($depth <= 0) {
        throw new InvalidArgumentException('Maksimal rekursionsdybde nået');
    }
    ...
    if (is_array($input)) {
        return array_map(function($value) use ($flags, $encoding, $depth) {
            return escapexss($value, $flags, $encoding, $depth - 1);
        }, $input);
    }

    if (is_object($input)) {
        // Håndter objekter hvis nødvendigt
        if (method_exists($input, '__toString')) {
            return htmlspecialchars((string) $input, $flags, $encoding);
        }
        // Eller konverter til array og proces
        $array = (array) $input;
        return (object) escapexss($array, $flags, $encoding, $depth - 1);
    }

    // For andre typer (int, float, bool, null) - returner uændret
    return $input;
}
...
function mailgf($to,$subject,$message,$headers=null,$parameters=null)
{
    ...
    try {
        ...
        $mail->setProtocol(Email::TLS)
            ->setLogin('GaveFabrikken A/S', 'md-r89RL13UhP4qieAhUjCF0w')
