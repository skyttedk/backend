...
class PostnordVarenrLog extends ActiveRecord\Model {
    static $table_name = "postnord_varenrlog";
    ...
    static public function createObject($data) {
        $obj = new PostnordVarenrLog($data);
        $obj->save();
        return ($obj);
    }

    static public function readObject($id) {
        $obj = PostnordVarenrLog::find($id);
        return ($obj);
    }

    static public function updateObject($data) {
        $obj = PostnordVarenrLog::find($data['id']);
